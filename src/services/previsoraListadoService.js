...
export const normalizePrevisoraListadoItem = (item = {}) => {
...
export const getCasosPrevisoraListadoPaginado = async ({ page = 1, limit = 100 } = {}) => {
  ...
  if (!response.ok || payload?.success === false) {
    throw new Error(payload?.error || `Error al obtener los casos del listado Previsora (${response.status})`);
  }
...
export const crearCasoPrevisoraListado = async (datos) => {
...
export const actualizarCasoPrevisoraListado = async (id, datos) => {
...
export const guardarLiquidadorEnCasoPrevisoraListado = async ({
  casoId,
  liquidador,
  casoBase = {},
}) => {
...
export const guardarInformeUnicoEnCasoPrevisoraListado = async ({
  casoId,
  informeUnico,
  casoBase = {},
}) => {
...
export const deleteCasoPrevisoraListado = async (id) => {
  if (!id) throw new Error('Identificador de caso no válido');
  const response = await fetch(`${API_URL}/${id}`, {
    method: 'DELETE',
    headers: authHeaders(),
...
export const importarCasosPrevisoraListado = async (casos = []) => {
...
export const subirArchivoPrevisoraListado = async (casoId, file, etiqueta = 'GENERAL', extras = {}) => {
...
export const eliminarArchivoPrevisoraListado = async (casoId, archivoId) => {
...
export const urlDescargaArchivoPrevisoraListado = (ruta) => resolveUploadsUrl(ruta);
