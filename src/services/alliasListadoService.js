...
const API_URL = `${BASE_URL}/api/allias-listado`;
...
export const normalizeAlliasListadoItem = (item = {}) => ({
  ...item,
  zc: item.zc ?? '',
...
const normalizeArray = (raw) =>
  Array.isArray(raw) ? raw.map((item) => normalizeAlliasListadoItem(item ?? {})) : [];

export const getCasosAlliasListadoPaginado = async ({ page = 1, limit = 100 } = {}) => {
  ...
  if (!response.ok || payload?.success === false) {
    throw new Error(payload?.error || `Error al obtener los casos del listado Allias (${response.status})`);
  }
...
export const fetchAllCasosAlliasListado = async (batchSize = 2000) => {
  const acumulado = [];
  let page = 1;
  let total = null;
  while (true) {
    const respuesta = await getCasosAlliasListadoPaginado({ page, limit: batchSize });
...
export const crearCasoAlliasListado = async (datos) => {
  ...
  return normalizeAlliasListadoItem(payload?.data ?? payload);
};

export const actualizarCasoAlliasListado = async (id, datos) => {
...
export const getCasoAlliasListadoById = async (id) => {
  if (!id) throw new Error('Identificador de caso no válido');
...
export const guardarLiquidadorEnCasoAlliasListado = async ({
  casoId,
  liquidador,
  casoBase = {},
}) => {
  if (!casoId) throw new Error('El caso del listado debe estar guardado antes de adjuntar el liquidador.');
  return actualizarCasoAlliasListado(casoId, {
    ...omitirMeta(casoBase),
    liquidador: liquidador || {},
  });
};

export const guardarInformeUnicoEnCasoAlliasListado = async ({
  casoId,
  informeUnico,
  casoBase = {},
}) => {
...
export const deleteCasoAlliasListado = async (id) => {
  if (!id) throw new Error('Identificador de caso no válido');
  const response = await fetch(`${API_URL}/${id}`, {
    method: 'DELETE',
    headers: authHeaders(),
...
export const importarCasosAlliasListado = async (casos = []) => {
