...
/**
 * Autoguardado del workspace Allias (liquidador / informe)
 ...
 */
export default function useAlliasCasoAutosave({
  casoId,
  casoAllias,
  tabActivo,
  liquidadorState,
  ...
  onCasoActualizado,
  enabled = true,
  guardarLiquidador = guardarLiquidadorEnCasoAllias,
  guardarInforme = guardarInformeUnicoEnCasoAllias,
} = {}) {
  const casoRef = useRef(casoAllias);
    ...
    const scheduleSave = (payload) => {
      ...
      const timer = setTimeout(async () => {
        ...
        try {
          ...
          console.error('Autoguardado Allias:', err);
          setAutosaveUiStatus({
            state: 'error',
