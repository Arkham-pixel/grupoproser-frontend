...
/**
 * Autoguardado del workspace BBVA CAT (liquidador / informe)
 ...
 */
export default function useBbvaCatCasoAutosave({
  casoId,
  casoBbvaCat,
  tabActivo,
  liquidadorState,
  ...
  onCasoActualizado,
  enabled = true,
  guardarLiquidador = guardarLiquidadorEnCasoBbvaCat,
  guardarInforme = guardarInformeUnicoEnCasoBbvaCat,
} = {}) {
  const casoRef = useRef(casoBbvaCat);
    ...
    const scheduleSave = (payload) => {
      ...
      const timer = setTimeout(async () => {
        ...
        try {
          ...
          console.error('Autoguardado BbvaCat:', err);
          setAutosaveUiStatus({
            state: 'error',
