...
import {
  ...
  normalizarGradoAfectacionEquidadCat,
  normalizarSiNoEquidadCat,
...
/** Mapeo de encabezados hoja BD / PENDIENTES / CAT_EQUIDAD → campos del caso. */
const HEADER_MAP = {
  SINIESTRO: 'siniestro',
  ...
  RESERVA: 'reserva',
  'RESERVA DIRECTA': 'reservaDirecta',
  'RESERVA GASTOS': 'reservaGastos',
  DIFERENCIA: 'diferenciaReserva',
  ...
  'DEDUCIBLE MAX': 'deducibleMaxPct',
  'DEDUCIBLE MAX %': 'deducibleMaxPct',
  'TIPO DEDUCIBLE': 'tipoDeducible',
  'DEDUCIBLE SMMLV': 'deducibleSmmlv',
  PRODUCTO: 'producto',
  CASO: 'numeroCasoCliente',
  'NO CASO': 'numeroCasoCliente',
  'N CASO': 'numeroCasoCliente',
  ANALISTA: 'analista',
  ASIGNACION: 'asignacion',
  'COMENTARIOS ANALISTA': 'comentariosAnalista',
  'COMENTARIO ANALISTA': 'comentariosAnalista',
  VISITA: 'visita',
  'YA SE ASIGNO A AJUSTADOR': 'asignadoAAjustador',
  ...
  'FECHA DEFINICION': 'fechaDefinicion',
  'FECHA DEFICNICION': 'fechaDefinicion',
...
const parseFechaCelda = (valor) => {
  ...
  if (mdy) {
    ...
    let month = Number(a);
    let day = Number(b);
    // Si el primero > 12, es dd/mm; si no, en consolidado EquidadCat suele ser m/d/yy (Excel US)
    if (month > 12 && day <= 12) {
      month = Number(b);
      day = Number(a);
    }
...
/** Evita materializar hojas con !ref inflado (p. ej. Hoja1 A1:AI1048576). */
const MAX_FILAS_EXCEL = 8000;

const ultimaFilaConCeldas = (sheet, tope = MAX_FILAS_EXCEL) => {
  let max = -1;
  for (const key of Object.keys(sheet)) {
    if (key.charAt(0) === '!') continue;
    const cell = XLSX.utils.decode_cell(key);
    if (cell.r > max) max = cell.r;
    if (max >= tope) break;
  }
  return max;
};

const matrizDesdeHoja = (sheet) => {
  const ref = sheet?.['!ref'];
  if (!ref) return [];
  const decoded = XLSX.utils.decode_range(ref);
  const filasRef = decoded.e.r - decoded.s.r + 1;
  let endR = decoded.e.r;
  if (filasRef > MAX_FILAS_EXCEL) {
    const real = ultimaFilaConCeldas(sheet);
    if (real < 0) return [];
    endR = Math.min(real, decoded.s.r + MAX_FILAS_EXCEL - 1);
  }
  const range = XLSX.utils.encode_range({
    s: decoded.s,
    e: { c: decoded.e.c, r: endR },
  });
  return XLSX.utils.sheet_to_json(sheet, { header: 1, defval: null, raw: true, range });
};

const parsearHojaACasos = (sheet) => {
  const matriz = matrizDesdeHoja(sheet);
  ...
  for (let r = headerRowIdx + 1; r < matriz.length; r += 1) {
    ...
    caso.estado = homologarEstadoEquidadCat(caso.estado);
...
const esHojaRellenoExcel = (nombre) => /^HOJA\s*\d+$/i.test(String(nombre || '').trim());
...
const elegirHojasCandidatas = (workbook) => {
  ...
  const baseEquidadCat = nombres.find((n) => /BASE EQUIDAD_CAT/i.test(n));
  const creciat = nombres.find((n) => /CRECIAT|CIAT/i.test(n));
  if (baseEquidadCat) orden.push(baseEquidadCat);
  if (creciat) orden.push(creciat);
  if (bd) orden.push(bd);
  if (pendientes) orden.push(pendientes);
  nombres.forEach((n) => {
    if (!orden.includes(n) && !esHojaRellenoExcel(n)) orden.push(n);
  });
  nombres.forEach((n) => {
...
const elegirHojasListado = (workbook) => {
  const nombres = workbook.SheetNames || [];
  const preferidas = nombres.filter((n) => /CRECIAT|CIAT|EQUIDAD/i.test(n));
  const resto = nombres.filter((n) => !preferidas.includes(n) && !esHojaRellenoExcel(n));
  const relleno = nombres.filter((n) => esHojaRellenoExcel(n));
  return [...preferidas, ...resto, ...relleno];
};
 ...
 */
export const parsearCasosEquidadCatDesdeExcel = async (file) => {
  ...
  if (/BASE EQUIDAD_CAT/i.test(hojaUsada)) {
    casos = casos.map((caso) => ({
      ...caso,
      causa: caso.causa || 'TERREMOTO',
      canalRadicacion: caso.canalRadicacion || 'EQUIDAD_CAT',
      tomador: caso.tomador || 'EQUIDAD_CAT SEGUROS',
    }));
  }
...
/** Columnas de Equidad que no se importan: las llena Grupo Proser (ajustador / inspector). */
const HEADERS_IGNORAR_LISTADO = new Set(['ITEM', 'AJUSTADOR', 'INSPECTOR']);
...
const parsearHojaListadoCliente = (sheet) => {
  ...
  for (let r = 0; r < Math.min(matriz.length, 20); r += 1) {
    ...
    row.forEach((celda, c) => {
      ...
      if (!header || HEADERS_IGNORAR_LISTADO.has(header)) return;
  ...
  for (let r = headerRowIdx + 1; r < matriz.length; r += 1) {
    ...
    Object.entries(colMap).forEach(([colStr, campo]) => {
      ...
      if (CAMPOS_FECHA.has(campo)) {
        const parsed = parseFechaCelda(raw) || '';
        if (parsed || !caso[campo]) caso[campo] = parsed;
        return;
      }
      if (CAMPOS_NUMERO.has(campo)) {
        const n = parseNumeroCelda(raw);
        if (n != null || caso[campo] == null || caso[campo] === '') caso[campo] = n;
        return;
      }
      const txt = limpiarTextoListado(raw);
      if (txt || !caso[campo]) caso[campo] = txt;
    });
    if (/^(pendiente|n\/?c)$/i.test(String(caso.fechaVisita || '').trim())) caso.fechaVisita = '';
    if (caso.celular && !caso.telefonoAsegurado) caso.telefonoAsegurado = caso.celular;
    if (!caso.celular && caso.telefonoAsegurado) caso.celular = caso.telefonoAsegurado;
    if (caso.valorAsegurado != null && caso.valorAseguradoInmueble == null) {
      caso.valorAseguradoInmueble = caso.valorAsegurado;
    }
    if (caso.valorIndemnizado != null && caso.valorLiquidado == null) {
      caso.valorLiquidado = caso.valorIndemnizado;
    }
    ...
    const extras = columnasSinMapa
      ...
      .filter((txt) => txt && !/^pendiente$/i.test(txt) && !/^\(?pagos\)?$/i.test(txt));
    if (extras.length) {
 ...
 */
export const parsearListadoClienteEquidadCatDesdeExcel = async (file) => {
  if (!file) throw new Error('No se seleccionó archivo');
  const buffer = await file.arrayBuffer();
  const workbook = XLSX.read(buffer, { type: 'array', cellDates: true });
  if (!workbook.SheetNames?.length) throw new Error('El archivo no contiene hojas válidas');
  ...
  for (const nombre of elegirHojasListado(workbook)) {
    const sheet = workbook.Sheets[nombre];
    if (!sheet) continue;
    const parseado = parsearHojaListadoCliente(sheet);
    if (parseado.casos.length > casos.length) {
      hojaUsada = nombre;
      casos = parseado.casos;
    }
  }
  ...
  if (!hojaUsada) {
    throw new Error(
      'No se encontró el listado de siniestros. Use el Excel de Equidad (SINIESTRO, CASO, ASEGURADO, PÓLIZA, CIUDAD).'
    );
  }
