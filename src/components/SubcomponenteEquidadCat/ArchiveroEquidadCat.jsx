...
import { ETIQUETAS_ARCHIVO_EQUIDAD_CAT, formatDate } from './equidadCatHelpers.js';
...
export default function ArchiveroEquidadCat({ caso, onClose, onChanged }) {
  ...
  const refrescar = async () => {
    const actualizado = await getCasoEquidadCatById(caso._id);
  ...
  const handleDelete = async (archivoId) => {
    if (!window.confirm(t('equidadCat.archive.confirmDelete'))) return;
    setError(null);
    setExito(null);
    try {
      await eliminarArchivoEquidadCat(caso._id, archivoId);
      await refrescar();
      setExito(t('equidadCat.archive.deleteOk'));
    } catch (err) {
      setError(err.message || t('equidadCat.archive.deleteError'));
    }
  };
          ...
          <SelectFenix value={etiqueta} onChange={(e) => setEtiqueta(e.target.value)}>
            {ETIQUETAS_ARCHIVO_EQUIDAD_CAT.map((op) => (
              <option key={op} value={op}>
                {t(`equidadCat.archive.labels.${op}`, { defaultValue: op })}
              </option>
            ))}
          </SelectFenix>
        </Campo>
          ...
          <thead className="bg-gray-50 dark:bg-gray-900/50">
            <tr>
              <th className="px-3 py-2 text-left font-body text-xs font-semibold uppercase text-gray-500">
                {t('equidadCat.archive.file')}
              </th>
              <th className="px-3 py-2 text-left font-body text-xs font-semibold uppercase text-gray-500">
                {t('equidadCat.archive.label')}
              </th>
              <th className="px-3 py-2 text-left font-body text-xs font-semibold uppercase text-gray-500">
                {t('equidadCat.archive.size')}
              </th>
              <th className="px-3 py-2 text-left font-body text-xs font-semibold uppercase text-gray-500">
                {t('equidadCat.archive.date')}
              </th>
            ...
            {archivos.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-3 py-6 text-center font-body text-sm text-gray-500">
                  {t('equidadCat.archive.empty')}
                </td>
              </tr>
            ) : (
              archivos.map((arch) => {
                const url = urlDescargaArchivoEquidadCat(arch.ruta);
                return (
                  <tr key={arch._id}>
                    ...
                    <td className="px-3 py-2 font-body text-sm text-gray-600 dark:text-gray-300">
                      {t(`equidadCat.archive.labels.${arch.etiqueta || 'GENERAL'}`, {
