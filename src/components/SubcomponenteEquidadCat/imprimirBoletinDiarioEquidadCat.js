...
/**
 * @param {object} boletin - resultado de calcularBoletinDiarioEquidadCat
 * @param {object} labels - textos ya resueltos (i18n)
 */
export function construirHtmlBoletinDiarioEquidadCat(boletin, labels = {}) {
...
<head>
  <meta charset="utf-8" />
  <title>${esc(L.docTitle || 'Boletín diario Equidad CAT')}</title>
  <style>${estilosImpresion()}</style>
</head>
<body>
  <p class="eyebrow">EQUIDAD CAT</p>
 ...
 */
export function imprimirBoletinDiarioEquidadCat(boletin, labels = {}) {
  const html = construirHtmlBoletinDiarioEquidadCat(boletin, labels);
