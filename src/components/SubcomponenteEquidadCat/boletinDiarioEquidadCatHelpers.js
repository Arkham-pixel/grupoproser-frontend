/**
 * Helpers del boletín diario Equidad CAT (corte día a día, America/Bogota).
 * Independiente del boletín semanal.
 */

import { homologarEstadoEquidadCat } from './equidadCatHelpers.js';

const TZ_INLINE = 'America/Bogota';
...
const CORTES_KEY = 'equidadCat.boletinDiario.cortes';
...
export function casoIdEquidadCat(caso) {
...
export function contarGestionTerremoto(casos = []) {
  ...
  for (const caso of Array.isArray(casos) ? casos : []) {
    ...
    const id = casoIdEquidadCat(caso);
...
export function fechaIngresoCasoEquidadCat(caso = {}) {
  return (
...
export function casoExistiaAlCorte(caso, isoCorte) {
  const ingreso = fechaIngresoCasoEquidadCat(caso);
...
export function calcularBoletinDiarioEquidadCat(casos = [], { fechaCorte = new Date(), persistirCorte = true } = {}) {
