...
import { fetchAllCasosEquidadCat, getCasoEquidadCatById } from '../../services/equidadCatService.js';
import ArchiveroEquidadCat from './ArchiveroEquidadCat.jsx';
import EquidadCatCasoPicker from './EquidadCatCasoPicker.jsx';

const root = 'min-h-full w-full min-w-0 bg-fenix-fondo dark:bg-[#0F0F0F] p-4 sm:p-6';

export default function ArchiveroEquidadCatPage() {
  ...
  useEffect(() => {
    ...
    async function cargar() {
      ...
      setError('');
      try {
        const actual = await getCasoEquidadCatById(casoIdFromQuery);
  ...
  const subtitulo = useMemo(() => {
    ...
    return t('equidadCat.archive.pageSubtitle');
  }, [caso, t]);
        ...
        <div className={expressCard}>
          <div className={expressCardBody}>
            {cargandoCaso ? (
              ...
              <ArchiveroEquidadCat caso={caso} onChanged={setCaso} />
            ) : (
              <p className="text-sm text-gray-500">{t('equidadCat.archive.pickCase')}</p>
            )}
          </div>
        </div>
      </div>
