 ...
 */
export async function generarLiquidadorZurichPdfBlob(liquidador) {
  const evalData = liquidador?.evaluacionSismicaNSR10 || {};
  const enc = liquidador?.encabezado || {};
  ...
  doc.setProperties({
    title: `Evaluación Sísmica NSR-10 Zurich - ${enc.siniestro || enc.asegurado || ''}`,
...
export async function descargarLiquidadorZurichPdf(liquidador, totales) {
  const blob = await generarLiquidadorZurichPdfBlob(liquidador, totales);
  const enc = liquidador?.encabezado || {};
