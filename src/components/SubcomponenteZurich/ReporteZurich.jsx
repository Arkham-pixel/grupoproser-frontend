...
export default function ReporteZurich() {
  ...
  const exportarExcel = () => {
    if (!casosOrdenados.length) {
      setAviso({ tipo: 'info', titulo: t('zurich.report.noData'), mensaje: t('zurich.report.noDataExport') });
      return;
    }
    try {
      ...
      setAviso({
        tipo: 'error',
        titulo: t('zurich.report.exportError'),
        mensaje: err.message || t('zurich.report.exportErrorMessage'),
      });
    }
  };
  ...
  const solicitarEliminar = (item) => {
      setAviso({
        ...
        onConfirm: async () => {
          try {
            await deleteCasoZurich(item._id);
            setAviso({
              tipo: 'success',
