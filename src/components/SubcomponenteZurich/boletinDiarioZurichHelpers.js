/**
 * Helpers del boletín diario Zurich (corte día a día, America/Bogota).
...
export function casoIdZurich(caso) {
...
export function clasificarCasoGestionTerremoto(caso = {}) {
  ...
  if (
    ...
    estado === 'PENDIENTE DOCUMENTOS (INFORME PRELIMINAR)' ||
    estado === 'LIQUIDAR (INFORME UNICO / FINAL)'
  ) {
    return 'enLiquidacion';
  }
  if (estado === 'INSPECCIÓN COORDINADA') return 'enInspeccion';
  return 'verificacion';
}
...
export function clasificarGestionDiscriminada(caso = {}) {
  ...
  if (
    estado === 'ANALISIS DEL CASO' ||
    estado === 'LIQUIDAR (INFORME UNICO / FINAL)' ||
    estado === 'AUTORIDAD DELEGADA'
  ) {
    return 'enLiquidacion';
  }
  if (estado === 'PENDIENTE DOCUMENTOS (INFORME PRELIMINAR)') return 'pendienteInformacion';
  if (estado === 'INSPECCIÓN COORDINADA' || estado === 'ASIGNADO') return 'solicitanInspeccion';
...
export function fechaIngresoCasoZurich(caso = {}) {
  return (
...
export function casoExistiaAlCorte(caso, isoCorte) {
  const ingreso = fechaIngresoCasoZurich(caso);
...
export function clasificarCasoAlCorte(caso = {}, isoCorte) {
  ...
  if (
    ...
    fechaIsoOnOrBefore(caso.fechaInformePreliminar, isoCorte) ||
    fechaIsoOnOrBefore(caso.fechaInformeFinal, isoCorte) ||
...
export function calcularBoletinDiarioZurich(casos = [], { fechaCorte = new Date(), persistirCorte = true } = {}) {
