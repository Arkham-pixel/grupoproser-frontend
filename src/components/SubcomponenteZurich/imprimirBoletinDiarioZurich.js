...
/**
 * @param {object} boletin - resultado de calcularBoletinDiarioZurich
 * @param {object} labels - textos ya resueltos (i18n)
 */
export function construirHtmlBoletinDiarioZurich(boletin, labels = {}) {
...
<head>
  <meta charset="utf-8" />
  <title>${esc(L.docTitle || 'Boletín diario Zurich')}</title>
  <style>${estilosImpresion()}</style>
</head>
<body>
  <p class="eyebrow">ZURICH</p>
 ...
 */
export function imprimirBoletinDiarioZurich(boletin, labels = {}) {
  const html = construirHtmlBoletinDiarioZurich(boletin, labels);
