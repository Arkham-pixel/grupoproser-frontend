...
import { importarCasosZurichListado } from '../../services/zurichListadoService.js';
import { parsearListadoClienteZurichDesdeExcel } from './importarZurichExcel.js';
import {
  expressBtnGhost,
  expressBtnPrimary,
} from '../SubcomponenteExpress/expressFenixUi.js';

export function esAdminOSoporteZurich() {
...
/**
 * Carga masiva Zurich: listado breve de cliente (ZC, STRO, asegurado, contactos, ciudad).
 * Empareja solo por ZC; no cruza los casos CAT de inspección.
 */
export default function ModalImportarExcelZurich({ open, onClose, onCompleted }) {
  ...
  const handleFile = async (selected) => {
    ...
    setAnalizando(true);
    try {
      const parsed = await parsearListadoClienteZurichDesdeExcel(selected);
      setHoja(parsed.hoja || '');
  ...
  const handleImport = async () => {
    ...
    setError(null);
    try {
      const data = await importarCasosZurichListado(casos);
      const totals = {
