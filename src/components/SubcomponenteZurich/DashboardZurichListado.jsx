...
import {
  aplicarFiltrosTorreZurich,
  clasificarAlertaZurich,
...
function kpiDinero(td, monto, count) {
  if (!count) {
    return { value: td('pendingLoad'), hint: td('kpis.moneyEmpty'), title: undefined, tone: undefined };
  }
  return {
...
export default function DashboardZurichListado() {
  ...
  const [config, setConfig] = useState(TORRE_CONFIG_ZURICH_DEFAULT);
  ...
  const { orden, cambiarOrden } = useOrdenTabla('zc', true);
    ...
    (async () => {
      ...
      try {
        const [data, cfg] = await Promise.all([
          fetchAllCasosZurichListado(),
          fetchTorreConfigZurich().catch(() => null),
        ]);
        if (cancelado) return;
        setCasos(Array.isArray(data) ? data : []);
        setConfig(fusionarConfigTorreZurich(cfg));
      } catch (err) {
        console.error('Error cargando torre listado Zurich:', err);
        if (!cancelado) {
  ...
  const casosFiltrados = useMemo(
    () => aplicarFiltrosTorreZurich(casos, filtros, config),
    [casos, filtros, config]
  );

  const hayFiltroFecha = Boolean(filtros.fechaDesde || filtros.fechaHasta);
  const stats = useMemo(
    () => construirTorreZurich(casosFiltrados, config, { granularidad, hayFiltroFecha }),
  ...
  const departamentos = useMemo(
    () => opcionesDesdeGetter(casos, (c) => resolverDepartamentoZurich(c)),
    [casos]
  );
  ...
  const etiquetaChip = (clave, valor) => {
    const mapa = {
      ...
      tipoPoliza: `${t('zurich.fields.tipoPoliza')}: ${valor}`,
      modalidad: `${t('zurich.fields.modalidadAtencion')}: ${valor}`,
      causa: `${t('zurich.fields.causa')}: ${valor}`,
      intermediario: `${t('zurich.fields.intermediario')}: ${valor}`,
  ...
  const filasDetalle = useMemo(() => {
    const filas = casosFiltrados.map((caso) => {
      const alerta = clasificarAlertaZurich(caso, config);
      return {
        ...caso,
        estado: homologarEstadoZurich(caso.estado),
        diasEstado: diasEnEstadoNumeroZurich(caso),
        diasTotal: diasAntiguedadTotalZurich(caso),
        tipoPolizaEtiqueta: etiquetaTipoPolizaZurich(caso),
  ...
  const totalPaginas = Math.max(1, Math.ceil(filasDetalle.length / ZURICH_REPORTE_PAGE_SIZE));
  const paginaActual = Math.min(pagina, totalPaginas);
  const paginaItems = filasDetalle.slice(
    (paginaActual - 1) * ZURICH_REPORTE_PAGE_SIZE,
    paginaActual * ZURICH_REPORTE_PAGE_SIZE
  );
  ...
  const exportarExcel = () => {
    const filas = filasDetalle.map((caso) => ({
      ...
      CIUDAD: caso.ciudad ?? '',
      DEPARTAMENTO: resolverDepartamentoZurich(caso),
      ESTADO: caso.estado ?? '',
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Torre Zurich');
    XLSX.writeFile(wb, `zurich-torre-${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
      ...
      <div className={`${expressPageWrap} min-w-0`}>
        <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-3">
            <span className={expressBadge}>{esInterno ? td('badgeStaff') : td('badge')}</span>
            <div>
                      ...
                      {paginaItems.map((row) => (
                        ...
                        >
                          <td className="px-4 py-3 font-medium">
                            {row._id ? (
                              <Link
                                to={`/zurich/listado/caso?casoId=${row._id}`}
                                className="text-fenix-primario hover:underline"
                              >
                                {row.zc || td('intervention.openCase')}
                              </Link>
                            ) : (
                              row.zc || '—'
                            )}
      ...
      <section className="overflow-hidden rounded-2xl border border-gray-100 bg-white dark:border-gray-800 dark:bg-[#1A1A1A]">
        <div className="grid grid-cols-1 divide-y divide-gray-100 sm:grid-cols-2 lg:grid-cols-4 sm:divide-x sm:divide-y-0 dark:divide-gray-800">
          ...
          <HeroKpi
            label={td('kpis.new')}
            value={kpis.nuevos}
            hint={hayFiltroFecha ? td('kpis.newHintFiltered') : td('kpis.newHintState')}
          />
          <HeroKpi
        ...
        <div className="grid grid-cols-1 divide-y divide-gray-100 border-t border-gray-100 sm:grid-cols-2 lg:grid-cols-4 sm:divide-x sm:divide-y-0 dark:divide-gray-800 dark:border-gray-800">
          <HeroKpi
            ...
            hint={kpis.casosConReserva ? td('kpis.reserveHint', { count: kpis.casosConReserva }) : td('kpis.reserveEmpty')}
            title={kpis.casosConReserva ? formatCurrency(kpis.reservaAbierta) : undefined}
          />
          <HeroKpi
            label={td('kpis.coverage')}
            value={kpis.coberturaReserva == null ? td('dash') : `${kpis.coberturaReserva}%`}
            hint={kpis.coberturaReserva == null ? td('kpis.coverageEmpty') : td('kpis.coverageHint')}
          />
          <HeroKpi
          ...
          />
          <HeroKpi
            label={td('kpis.evacuation')}
            value={kpis.indiceEvacuacion == null ? td('dash') : `${kpis.indiceEvacuacion}%`}
            hint={
              kpis.indiceEvacuacion == null
                ? td('kpis.evacuationEmpty')
                : td('kpis.evacuationHint', { out: kpis.finalizadosPeriodo, in: kpis.ingresosPeriodo })
            }
          />
        </div>
      </section>
          ...
          <ExpressChartPlot height={Math.max(280, stats.pareto.length * 28)}>
            <ComposedChart data={stats.pareto} layout="vertical" margin={{ top: 4, right: 16, left: 4, bottom: 4 }}>
              ...
              <Bar
                ...
                cursor="pointer"
                onClick={(entry) => patchFiltro('busqueda', entry?.zc || entry?.payload?.zc || '')}
              />
            </ComposedChart>
          </ExpressChartPlot>
        </ChartCard>
    ...
    <>
      <section className="overflow-hidden rounded-2xl border border-gray-100 bg-white dark:border-gray-800 dark:bg-[#1A1A1A]">
        <div className="grid grid-cols-1 divide-y divide-gray-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0 dark:divide-gray-800">
          <HeroKpi
            label={td('kpis.reserve')}
            value={kpis.casosConReserva ? formatCurrency(kpis.reservaAbierta) : td('pendingLoad')}
            hint={kpis.casosConReserva ? td('kpis.reserveHint', { count: kpis.casosConReserva }) : td('kpis.reserveEmpty')}
            title={kpis.casosConReserva ? formatCurrency(kpis.reservaAbierta) : undefined}
          />
          <HeroKpi
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {stats.topReservas.map((row) => (
                  <tr key={String(row.id || row.zc)} className="border-t border-gray-100 dark:border-gray-800">
                    <td className="px-4 py-3 font-medium">
                      {row.id ? (
                        <Link to={`/zurich/listado/caso?casoId=${row.id}`} className="text-fenix-primario hover:underline">
                          {row.zc || td('intervention.openCase')}
                        </Link>
                      ) : (
                        row.zc || '—'
