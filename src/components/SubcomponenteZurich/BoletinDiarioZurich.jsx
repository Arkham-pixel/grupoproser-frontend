...
export default function BoletinDiarioZurich() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelado) {
          setError(err.message || t('zurich.boletinDiario.loadError'));
          setCasos([]);
        }
      } finally {
  ...
  const boletin = useMemo(
    () =>
      calcularBoletinDiarioZurich(casos, {
        fechaCorte,
  ...
  const handleImprimirPdf = () => {
    imprimirBoletinDiarioZurich(boletin, {
      docTitle: `${t('zurich.boletinDiario.title')} · ${boletin.etiquetaHoyCorta}`,
      title: t('zurich.boletinDiario.title'),
      subtitle: t('zurich.boletinDiario.subtitle', { fecha: boletin.etiquetaHoy }),
      terremotoTitle: t('zurich.boletinDiario.terremoto.title'),
      terremotoSubtitle: t('zurich.boletinDiario.terremoto.subtitle'),
      totalGeneral: t('zurich.boletinDiario.terremoto.total'),
      cutOff: t('zurich.boletinDiario.cutOff'),
      comparativoTitle: t('zurich.boletinDiario.comparativo.title'),
      comparativoSubtitle: t('zurich.boletinDiario.comparativo.subtitle'),
      cutLabel: t('zurich.boletinDiario.comparativo.cutLabel', {
        ayer: boletin.etiquetaAyer,
        hoy: boletin.etiquetaHoy,
      }),
      estado: t('zurich.boletinDiario.comparativo.estado'),
      yesterday: t('zurich.boletinDiario.comparativo.yesterday'),
      today: t('zurich.boletinDiario.comparativo.today'),
      variation: t('zurich.boletinDiario.comparativo.variation'),
      effectiveYesterday: t('zurich.boletinDiario.comparativo.effectiveYesterday'),
      effectiveToday: t('zurich.boletinDiario.comparativo.effectiveToday'),
      newAssignments: t('zurich.boletinDiario.comparativo.newAssignments'),
      baseGrowth: t('zurich.boletinDiario.comparativo.baseGrowth', {
        pct: comparativo.pctIncrementoBase ?? 0,
      }),
      netManaged: t('zurich.boletinDiario.comparativo.netManaged'),
      regressions: t('zurich.boletinDiario.comparativo.regressions'),
      noRegressions: t('zurich.boletinDiario.comparativo.noRegressions'),
      hasRegressions: t('zurich.boletinDiario.comparativo.hasRegressions'),
      summaryBanner: t('zurich.boletinDiario.comparativo.summaryBanner', {
        desde: comparativo.gestionAyer,
        hasta: comparativo.gestionHoy,
        nuevas: comparativo.nuevasAsignaciones,
      }),
      summaryText: t('zurich.boletinDiario.comparativo.summaryText', {
        pctAyer: comparativo.pctGestionAyer,
        pctHoy: comparativo.pctGestionHoy,
        neto: comparativo.incrementoGestion,
      }),
      discEyebrow: t('zurich.boletinDiario.discriminada.eyebrow'),
      discTitle: t('zurich.boletinDiario.discriminada.title'),
      discSubtitle: t('zurich.boletinDiario.discriminada.subtitle'),
      discSource: t('zurich.boletinDiario.discriminada.source', {
      ...
      <div className={expressPageWrap}>
        <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between print:block">
          <div>
            ...
            <h1 className={expressPageTitle}>{t('zurich.boletinDiario.title')}</h1>
            <p className={expressPageSubtitle}>
              {t('zurich.boletinDiario.subtitle', { fecha: boletin.etiquetaHoy })}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2 print:hidden">
            ...
            >
              <FaChevronLeft /> {t('zurich.boletinDiario.prevDay')}
            </button>
            <button
              type="button"
              className={expressBtnGhost}
            ...
            >
              {t('zurich.boletinDiario.nextDay')} <FaChevronRight />
            </button>
            <button type="button" className={expressBtnPrimary} onClick={handleImprimirPdf}>
              <FaPrint /> {t('zurich.boletinDiario.print')}
            </button>
          ...
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="font-heading text-xl font-bold tracking-wide text-gray-900 dark:text-white">
                {t('zurich.boletinDiario.terremoto.title')}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {t('zurich.boletinDiario.terremoto.subtitle')}
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-2 rounded-full border-2 border-fenix-primario px-4 py-1.5 text-sm font-bold text-fenix-primario dark:border-red-400 dark:text-red-300">
                {t('zurich.boletinDiario.terremoto.total')}: {gestionTerremoto.total}
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-lg bg-red-50 px-3 py-1.5 text-xs font-medium text-fenix-primario dark:bg-red-950/50 dark:text-red-200">
                <FaCalendarAlt />
                {t('zurich.boletinDiario.cutOff')}: {boletin.etiquetaHoy}
              </span>
            </div>
          </div>
        ...
        <section className="rounded-2xl border border-fenix-borde bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-950 print:break-before-page">
          <div className="mb-4">
            <h2 className="font-heading text-xl font-bold tracking-wide text-gray-900 dark:text-white">
              {t('zurich.boletinDiario.comparativo.title')}
            </h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {t('zurich.boletinDiario.comparativo.subtitle')}
            </p>
            <p className="mt-1 text-xs text-gray-400">
              {t('zurich.boletinDiario.comparativo.cutLabel', {
                ayer: boletin.etiquetaAyer,
                hoy: boletin.etiquetaHoy,
              })}
            </p>
          ...
          {!comparativo.tieneAyer && (
            <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-100">
              {t('zurich.boletinDiario.comparativo.noYesterday', {
          ...
          {comparativo.tieneAyer && comparativo.fuenteAyer === 'reconstruido' && (
            <div className="mb-4 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-900 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-100">
              {t('zurich.boletinDiario.comparativo.reconstructedNote', {
                fecha: boletin.etiquetaAyerCorta,
              })}
            </div>
          )}
              ...
              <thead>
                <tr>
                  <th className="border border-gray-200 bg-gray-100 px-3 py-2 text-left font-semibold text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200">
                    {t('zurich.boletinDiario.comparativo.estado')}
                  </th>
                  <th className="border border-gray-200 bg-gray-100 px-3 py-2 text-center font-semibold text-gray-700 dark:border-gray-700 dark:bg-gray-800">
                    {t('zurich.boletinDiario.comparativo.yesterday')}
                  ...
                  <th className="border border-red-100 bg-red-50 px-3 py-2 text-center font-semibold text-fenix-primario dark:border-red-900/40 dark:bg-red-950/40 dark:text-red-200">
                    {t('zurich.boletinDiario.comparativo.variation')}
                  </th>
                  <th className="border border-fenix-primario bg-fenix-primario px-3 py-2 text-center font-semibold text-white dark:border-red-700 dark:bg-red-700">
                    {t('zurich.boletinDiario.comparativo.today')}
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                {t('zurich.boletinDiario.comparativo.effectiveYesterday')}
              </p>
            ...
            <div className="rounded-xl border border-red-100 bg-red-50 p-4 dark:border-red-900/40 dark:bg-red-950/40">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-fenix-primario dark:text-red-300">
                {t('zurich.boletinDiario.comparativo.effectiveToday')}
              </p>
              <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">
            ...
            <div className="rounded-xl border border-fenix-borde p-4 dark:border-gray-700">
              ...
              <p className="text-xs font-semibold uppercase text-gray-600 dark:text-gray-300">
                {t('zurich.boletinDiario.comparativo.newAssignments')}
              </p>
              <p className="mt-1 text-xs text-gray-500">
                {comparativo.pctIncrementoBase != null
                  ? t('zurich.boletinDiario.comparativo.baseGrowth', {
            ...
            <div className="rounded-xl border border-fenix-borde p-4 dark:border-gray-700">
              ...
              <p className="text-xs font-semibold uppercase text-gray-600 dark:text-gray-300">
                {t('zurich.boletinDiario.comparativo.netManaged')}
              </p>
              <p className="mt-1 text-xs text-gray-500">
            ...
            <div className="rounded-xl border border-fenix-borde p-4 dark:border-gray-700">
              ...
              <p className="text-xs font-semibold uppercase text-gray-600 dark:text-gray-300">
                {t('zurich.boletinDiario.comparativo.regressions')}
              </p>
              <p className="mt-1 text-xs text-gray-500">
                {comparativo.retrocesos === 0
                  ? t('zurich.boletinDiario.comparativo.noRegressions')
                  : t('zurich.boletinDiario.comparativo.hasRegressions')}
              </p>
            </div>
          </div>

          <div className="mt-5 rounded-xl bg-fenix-primario px-4 py-3 text-sm font-semibold text-white dark:bg-red-800">
            {t('zurich.boletinDiario.comparativo.summaryBanner', {
              desde: comparativo.gestionAyer,
              hasta: comparativo.gestionHoy,
              nuevas: comparativo.nuevasAsignaciones,
            })}
          ...
          <p className="mt-3 text-sm leading-relaxed text-gray-600 dark:text-gray-300">
            {t('zurich.boletinDiario.comparativo.summaryText', {
              pctAyer: comparativo.pctGestionAyer,
              pctHoy: comparativo.pctGestionHoy,
              neto: comparativo.incrementoGestion,
            })}
        ...
        <section className="rounded-2xl border border-fenix-borde bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-950 print:break-before-page">
          <div className="mb-2 text-center">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-400">
              {t('zurich.boletinDiario.discriminada.eyebrow')}
            </p>
            <div className="mx-auto mt-2 inline-block rounded-xl bg-fenix-primario px-6 py-3 text-lg font-bold text-white dark:bg-red-700">
              {t('zurich.boletinDiario.discriminada.title')}: {discriminada.total}
            </div>
            <p className="mt-2 text-sm italic text-gray-500 dark:text-gray-400">
              {t('zurich.boletinDiario.discriminada.subtitle')}
            </p>
          </div>
          ...
          <p className="mt-5 text-center text-xs text-gray-400">
            {t('zurich.boletinDiario.discriminada.source', {
              hoy: boletin.etiquetaHoyCorta,
              ayer: boletin.etiquetaAyerCorta,
            })}
          </p>
