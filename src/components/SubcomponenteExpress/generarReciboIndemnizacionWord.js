...
import { buildReciboPreview } from './liquidadorExpressHelpers.js';
...
export async function descargarReciboIndemnizacionWord(liquidador, totales) {
  ...
  const descStro =
    liquidador?.checklist?.descripcionEvento?.trim() ||
    '[descripción del siniestro — editar manualmente]';
  ...
  saveAs(blob, `Recibo_Indemnizacion_${reclamoSafe}.docx`);
}
