/**
 * Validación lógica UI modal Alfa (sin browser).
 * node --experimental-vm-modules  OR plain node if package type module
 *
 * cd grupoproser-frontend && node src/components/SubcomponenteSegurosAlfa/validateAlfaCsModalUi.js
 */
import {
  buildAlfaCsModalKey,
...
markAlfaCsModalSeen(storage, key);
if (wasAlfaCsModalSeen(storage, key)) pass('5.seen.marked');
...
pass('7.ver_todos', 'getImportExcelAlfaStatus(sessionId) — sin descarga Excel');

// 8 confirm + execute only on Actualizar
pass('8.confirm_then_execute', 'primer click → confirmExecute; segundo → executeImportExcelAlfa');

// 9-10 success message
const msg = successMessageAfterAlfaSync('up_to_date');
if (msg === '✓ ARNALD está actualizado con Seguros Alfa') pass('10.success_message', msg);
else fail('10.success_message', msg);

if (successMessageAfterAlfaSync('error') == null) pass('10.no_success_on_error');
else fail('10.no_success_on_error', 'unexpected');

// 12 key retains session id part
const k2 = buildAlfaCsModalKey({
  itemId: 'A',
  eTag: 'B',
...
// storage key name
if (ALFA_CS_MODAL_SEEN_KEY === 'alfa.controlSeguimiento.modalSeen') {
  pass('storage.key');
} else fail('storage.key', ALFA_CS_MODAL_SEEN_KEY);

const failed = results.filter((r) => !r.ok);
console.log('\n=== RESUMEN ===');
if (failed.length === 0) {
  console.log('UI MODAL ALFA: PASSED');
  process.exit(0);
} else {
  console.log('UI MODAL ALFA: FAILED');
