 ...
 * Logo Proser | Título + subtítulo + código/versión/fecha | Logo Alfa
 */
async function crearEncabezadoAlfa({ caso = {}, informe = {} } = {}) {
        ...
        rows: [
          new TableRow({
            children: [
              ...
              logoCell(alfa, 'SEGUROS ALFA', AlignmentType.RIGHT),
            ],
          }),
        ],
      }),
...
async function resolverBytesFoto(foto, archivosCaso = []) {
  ...
  // blob: local ANTES que red (misma sesión = Word rápido sin S3)
...
async function construirBloqueDaniosUbicacionAlfa({ info = {}, caso = {} } = {}) {
  const bloques = [];
...
/**
 * Zona de firmas Alfa: solo ajustador, sin tabla/bordes, abajo a la izquierda.
 */
async function construirZonaFirmasAlfa({ info = {} } = {}) {
...
/**
 * Informe único Seguros Alfa — misma fórmula visual que Catastrófico/Puertos:
 * encabezado formal, título ÚNICO, cuadro ficha, secciones y cuadros sin relleno.
 */
export async function descargarWordInformeAlfa({ caso = {}, informe = null, liquidador = null } = {}) {
  const info = informe || defaultInformeUnicoAlfa(caso);
  const liq = liquidador || mapCasoAlfaALiquidador(caso);
  ...
  const mapaEvento = await loadLogoBytes(
    `${baseUrl}templates/mapa-evento-siniestro-alfa.png`
  );
  ...
  const header = await crearEncabezadoAlfa({ caso, informe: info });
  ...
  const firmasParrafos = await construirZonaFirmasAlfa({ caso, enc, info });
  const bloqueDaniosUbicacion = await construirBloqueDaniosUbicacionAlfa({ info, caso });
