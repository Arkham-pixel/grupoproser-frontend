...
/**
 * Cabecera con logos grandes visibles: Proser izq. | título | Alfa der.
...
function prepararCabeceraConLogos(sheet, {
  ...
  return { colAlfa, anchoProser, altoProser, anchoAlfa, altoAlfa };
}
...
function colocarLogosEnHoja(sheet, logoIds, layout) {
  ...
  if (logoIds.alfaId != null) {
    sheet.addImage(logoIds.alfaId, {
      tl: { col: colAlfa, row: 0.05 },
      ext: { width: anchoAlfa, height: altoAlfa },
      editAs: 'oneCell',
    });
  }
}

async function registrarLogosAlfaProser(workbook) {
  ...
  const alfa = await cargarLogoBuffer(`${BASE_PUBLIC}templates/logo-seguros-alfa.png`);
  ...
  if (alfa) {
    logoIds.alfaId = workbook.addImage({
      buffer: alfa.buffer,
      extension: alfa.extension,
    });
  }
  return logoIds;
}
 ...
 * con los datos del liquidador Alfa.
...
/**
 * Excel liquidador Alfa = plantilla oficial Evaluación Sísmica NSR-10 rellenada.
 */
export async function generarLiquidadorAlfaExcelBlob(liquidador) {
  ...
  if (hojaPortada) {
    ...
    if (logoIds.alfaId != null) {
      hojaPortada.addImage(logoIds.alfaId, {
        tl: { col: 2.4, row: 2.05 },
  ...
  return {
    ...
    filename: `Evaluacion_Sismica_NSR10_Alfa_${safe}.xlsx`,
  };
}

export async function descargarLiquidadorAlfaExcel(liquidador, totales) {
  const { blob, filename } = await generarLiquidadorAlfaExcelBlob(liquidador, totales);
