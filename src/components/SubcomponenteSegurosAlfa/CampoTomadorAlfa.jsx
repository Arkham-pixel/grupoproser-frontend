...
import {
  agregarTomadorExtraAlfa,
  eliminarTomadorExtraAlfa,
  esTomadorDefaultAlfa,
  leerTomadoresExtraAlfa,
  listarTomadoresAlfa,
  normalizarTomadorAlfa,
...
const CampoTomadorAlfa = ({
  ...
  const opciones = useMemo(() => listarTomadoresAlfa(extras), [extras]);
  const valorActual = String(value || '').trim();
  const valorHuerfano =
    valorActual &&
    !opciones.some((op) => normalizarTomadorAlfa(op) === normalizarTomadorAlfa(valorActual));
  ...
  const handleAgregar = () => {
    const nombre = normalizarTomadorAlfa(nuevo);
    setAviso('');
    if (!nombre) {
      setAviso(t('segurosAlfa.tomadores.empty'));
      return;
    }
    if (opciones.some((op) => normalizarTomadorAlfa(op) === nombre)) {
      setAviso(t('segurosAlfa.tomadores.alreadyExists'));
      onChange?.(nombre);
      setNuevo('');
      return;
    }
    const next = agregarTomadorExtraAlfa(nombre);
    setExtras(next);
    onChange?.(nombre);
    setNuevo('');
  };
  ...
  const handleEliminar = (nombre) => {
    if (esTomadorDefaultAlfa(nombre)) return;
    const next = eliminarTomadorExtraAlfa(nombre);
    setExtras(next);
    if (normalizarTomadorAlfa(valorActual) === normalizarTomadorAlfa(nombre)) {
      onChange?.('');
    }
  };
      ...
      {mostrarGestion && (
        <div className="rounded-lg border border-gray-200 bg-white/60 p-3 dark:border-gray-700 dark:bg-gray-900/40">
          <p className="mb-2 font-body text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
            {t('segurosAlfa.tomadores.manageTitle')}
          </p>
          <p className="mb-3 font-body text-xs text-gray-500 dark:text-gray-400">
            {t('segurosAlfa.tomadores.manageHint')}
          </p>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
            <div className="min-w-0 flex-1">
              <InputFenix
                ...
                placeholder={t('segurosAlfa.tomadores.newPlaceholder')}
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
                    handleAgregar();
              ...
              {extras.map((nombre) => (
                ...
                >
                  ...
                  <button
                    ...
                    title={t('segurosAlfa.tomadores.remove')}
                    aria-label={t('segurosAlfa.tomadores.remove')}
...
export default CampoTomadorAlfa;
