...
import { homologarEstadoAlfa } from './segurosAlfaHelpers.js';
import {
  isoDateBogota,
  parseFechaCaso,
  sumarDiasUtc,
...
export function clasificarCasoGestionTerremoto(caso = {}) {
  ...
  if (estado === 'DESISTIDO') return 'desistimientos';
  if (estado === 'OBJETADO') return 'objetados';
  if (esPerdidaTotalTexto(textoLibre, estado)) return 'perdidasTotales';
  if (estado === 'LIQUIDADO') return 'liquidados';
  if (estado === 'ENVIADO ASEGURADORA' || estado === 'CERRADO') return 'pendientesPagoAlfa';
  if (estado === 'Inspeccionado' || estado === 'Solicitud de documentos') return 'enLiquidacion';
  if (estado === 'Contactado y programado') return 'enInspeccion';
  return 'verificacion';
}
...
export function clasificarGestionDiscriminada(caso = {}) {
  ...
  if (estado === 'Contactado y programado') return 'solicitanInspeccion';
