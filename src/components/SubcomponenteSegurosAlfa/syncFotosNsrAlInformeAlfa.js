/**
 * Fotos por fila del checklist NSR-10 → informe único / archivero Alfa.
 */
...
import { defaultInformeUnicoAlfa } from './liquidadorAlfaHelpers.js';
...
export async function subirFotoFilaNsrAlfa({ casoId, file, item }) {
  ...
  if (creado?._id) {
    try {
      await actualizarArchivoAlfa(casoId, creado._id, { descripcion });
    } catch {
      /* no bloqueante */
    }
  }
...
export async function sincronizarFotosNsrEnInformeCaso({
  ...
  const baseInforme = defaultInformeUnicoAlfa(casoBase);
  const guardado =
  ...
  return guardarInformeUnicoEnCasoAlfa({
    casoId,
