 ...
 */
export async function generarLiquidadorAlfaPdfBlob(liquidador) {
  const evalData = liquidador?.evaluacionSismicaNSR10 || {};
  const enc = liquidador?.encabezado || {};
  ...
  doc.setProperties({
    title: `Evaluación Sísmica NSR-10 Alfa - ${enc.siniestro || enc.asegurado || ''}`,
...
export async function descargarLiquidadorAlfaPdf(liquidador, totales) {
  const blob = await generarLiquidadorAlfaPdfBlob(liquidador, totales);
  const enc = liquidador?.encabezado || {};
  const safe = String(enc.siniestro || enc.consecutivo || 'NSR10')
    .replace(/[^\w.-]+/g, '_')
    .slice(0, 40);
  const nombre = `Evaluacion_Sismica_NSR10_Alfa_${safe}.pdf`;
  saveAs(blob, nombre);
  return { blob, nombre };
}
