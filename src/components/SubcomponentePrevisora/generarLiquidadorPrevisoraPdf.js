...
function resolverPortada(liquidador) {
  ...
  return fusionarPortadaConFormData(evalData.portada || {}, {
    ...prefillNsrDesdecasoPrevisora({}, enc),
    ...enc,
 ...
 */
export async function generarLiquidadorPrevisoraPdfBlob(liquidador) {
  const evalData = liquidador?.evaluacionSismicaNSR10 || {};
  const enc = liquidador?.encabezado || {};
  ...
  doc.setProperties({
    title: `Evaluación Sísmica NSR-10 Previsora - ${enc.siniestro || enc.asegurado || ''}`,
...
export async function descargarLiquidadorPrevisoraPdf(liquidador, totales) {
  const blob = await generarLiquidadorPrevisoraPdfBlob(liquidador, totales);
  const enc = liquidador?.encabezado || {};
