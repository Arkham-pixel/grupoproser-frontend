...
import { buildCatastroficoNavStateDesdePrevisora } from './prefillCatastroficoDesdePrevisora.js';
import DisclaimerCatPrevisora from './DisclaimerCatPrevisora.jsx';
import InformeUnicoPrevisora from './InformeUnicoPrevisora.jsx';
import {
  ACCESO_PREDIO_PREVISORA,
  EVIDENCIA_CAT_KEYS,
  EVIDENCIA_CAT_VACIA,
 ...
 */
export default function InformeCatastroficoPrevisora({
  casoPrevisora = null,
  onCasoChange,
  onEstadoChange,
  onLiquidadorChange,
  ...
  const [severidadCat, setSeveridadCat] = useState(
    casoPrevisora?.severidadCat != null ? String(casoPrevisora.severidadCat) : ''
  );
  const [accesoPredio, setAccesoPredio] = useState(casoPrevisora?.accesoPredio || '');
  const [observacionesCat, setObservacionesCat] = useState(casoPrevisora?.observacionesCat || '');
  const [evidenciaCat, setEvidenciaCat] = useState(() =>
    normalizeEvidenciaCat(casoPrevisora?.evidenciaCat)
  );

  useEffect(() => {
    setSeveridadCat(casoPrevisora?.severidadCat != null ? String(casoPrevisora.severidadCat) : '');
    setAccesoPredio(casoPrevisora?.accesoPredio || '');
    setObservacionesCat(casoPrevisora?.observacionesCat || '');
    setEvidenciaCat(normalizeEvidenciaCat(casoPrevisora?.evidenciaCat));
  }, [casoPrevisora?._id, casoPrevisora?.updatedAt]);
  ...
  const guardarCat = async () => {
    ...
    try {
      ...
      const actualizado = await actualizarCasoPrevisora(casoPrevisora._id, payload);
      onCasoChange?.(actualizado);
      setMensaje(t('previsora.cat.savedOk'));
    } catch (err) {
      setError(err.message || t('previsora.cat.savedError'));
    } finally {
  ...
  const abrirCatastroficoComplex = async () => {
    ...
    try {
      const casoConCat = {
        ...casoPrevisora,
        severidadCat: severidadCat ? Number(severidadCat) : casoPrevisora.severidadCat,
        accesoPredio: accesoPredio || casoPrevisora.accesoPredio,
        observacionesCat: observacionesCat || casoPrevisora.observacionesCat,
        evidenciaCat: normalizeEvidenciaCat(evidenciaCat),
      };
      const stateRetorno = buildCatastroficoNavStateDesdePrevisora(
        casoConCat,
        `/previsora/caso?casoId=${casoPrevisora._id}&tab=informe`
      );

      let idExistente = casoPrevisora.historialCatastroficoId || null;
      if (!idExistente) {
        const historial = await historialService.obtenerHistorial({
          tipo: TIPOS_FORMULARIOS.CATASTROFICO,
          limite: 1000,
        });
        const clave = normalizarClaveCaso(casoPrevisora.consecutivo || casoPrevisora.siniestro);
        const mismoCaso = (Array.isArray(historial) ? historial : [])
          .filter((f) => {
            const posibles = [
              f?.numeroCaso,
              f?.datos?.numeroCaso,
              f?.datos?.previsoraCasoId,
            ]
              .map(normalizarClaveCaso)
              .filter(Boolean);
            return (
              posibles.includes(clave) ||
              String(f?.datos?.previsoraCasoId || '') === String(casoPrevisora._id)
            );
          })
          .sort((a, b) => {
      ...
      if (idExistente && idExistente !== casoPrevisora.historialCatastroficoId) {
        try {
          const actualizado = await actualizarCasoPrevisora(casoPrevisora._id, {
            ...casoPrevisora,
      ...
      setError(err.message || t('previsora.reportUnique.openError'));
    } finally {
      setAbriendo(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ——— CAT (Manual Previsora) ——— */}
      <section className={expressFormSection}>
        <h3 className={expressSectionTitle}>{t('previsora.sections.catInspection')}</h3>
        <DisclaimerCatPrevisora className="mb-4" />

        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          <Campo label={t('previsora.fields.severidadCat')}>
          ...
          <Campo label={t('previsora.fields.accesoPredio')}>
            <SelectFenix value={accesoPredio} onChange={(e) => setAccesoPredio(e.target.value)}>
              <option value="">{t('common.select')}</option>
              {ACCESO_PREDIO_PREVISORA.map((op) => (
                <option key={op} value={op}>
                  {t(`previsora.cat.acceso.${op}`, { defaultValue: op })}
                </option>
              ))}
            </SelectFenix>
          </Campo>
        ...
        <div className="mt-4 space-y-3">
          <p className="mb-1 font-body text-sm font-semibold text-gray-700 dark:text-gray-200">
            {t('previsora.cat.evidenciaTitle')}
          </p>
            ...
            return (
              ...
              >
                <p className="font-body text-sm font-semibold text-gray-800 dark:text-gray-100">
                  {t(`previsora.cat.evidencia.${item.key}`)}
                </p>
                <p className="mb-2 font-body text-xs text-gray-500 dark:text-gray-400">
                  {t(`previsora.cat.evidenciaCuando.${item.obligatorio}`)}
                </p>
        ...
        <div className="mt-4">
          <Campo label={t('previsora.fields.observacionesCat')}>
            <textarea
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <button
            ...
            onClick={guardarCat}
            disabled={guardandoCat || !casoPrevisora?._id}
          >
            <FaSave className="mr-2 inline" />
            {guardandoCat ? t('previsora.actions.saving') : t('previsora.cat.saveCat')}
          </button>
          <button
            type="button"
            className={expressBtnSecondary}
            onClick={abrirCatastroficoComplex}
            disabled={abriendo || !casoPrevisora?._id}
          >
            <FaFileAlt className="mr-2 inline" />
            {t('previsora.reportUnique.openCatastrofico')}
            <FaExternalLinkAlt className="ml-2 inline text-xs opacity-80" />
          </button>
          <button
            type="button"
            className={expressBtnGhost}
            onClick={() => navigate('/previsora/reporte')}
          >
            {t('previsora.workspace.backToReport')}
          </button>
        </div>
      </section>
      ...
      <section className={expressFormSection}>
        <h3 className={expressSectionTitle}>{t('previsora.workspace.tabUniqueReport')}</h3>
