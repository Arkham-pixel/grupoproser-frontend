/**
 * Helpers del boletín diario Previsora (corte día a día, America/Bogota).
 * Independiente del boletín semanal.
 */

import { homologarEstadoPrevisora } from './previsoraHelpers.js';
import {
  isoDateBogota,
  parseFechaCaso,
  sumarDiasUtc,
...
const CORTES_KEY = 'previsora.boletinDiario.listado.cortes';
...
export function casoIdPrevisora(caso) {
...
export function clasificarGestionDiscriminada(caso = {}) {
  ...
  if (texto.includes('desist') || estado === 'ANULADO' || estado === 'DESISTIDO' || estado === 'DESISTIMIENTO') {
...
export function fechaIngresoCasoPrevisora(caso = {}) {
  return (
...
export function casoExistiaAlCorte(caso, isoCorte) {
  const ingreso = fechaIngresoCasoPrevisora(caso);
...
export function clasificarCasoAlCorte(caso = {}, isoCorte) {
  ...
  if (
    fechaIsoOnOrBefore(caso.fechaPresentacionCifras || caso.fechaAnalisisCaso, isoCorte) ||
  ...
  if (
    fechaIsoOnOrBefore(caso.fechaCasoInspeccionado || caso.fechaCoordinandoInspeccion, isoCorte) ||
...
export function calcularBoletinDiarioPrevisora(casos = [], { fechaCorte = new Date(), persistirCorte = true } = {}) {
