...
export function homologarEstadoPrevisora(valor) {
  ...
  const key = claveEstadoPrevisora(raw);
  const exacto = ESTADOS_PREVISORA.find((est) => claveEstadoPrevisora(est) === key);
...
export function ultimaGestionPrevisora(caso = {}) {
  const claves = [
    ...CAMPOS_FECHA_ACCION_PREVISORA,
    'fechaAsignacion',
    'fechaVisita',
    'fechaInspeccion',
...
export const TIPOS_IDENTIFICACION_PREVISORA = [
  'CC',
  'CE',
  'NIT',
  'PASAPORTE',
...
export const TIPOS_POLIZA_PREVISORA = [
  'HOGAR',
  'INCENDIO',
  'TERREMOTO',
  'TODO RIESGO',
...
export const esTipoPolizaOtroPrevisora = (valor) =>
  /^OTROS?$/.test(String(valor || '').trim().toUpperCase());

export const etiquetaTipoPolizaPrevisora = (caso = {}) => {
  const tipo = String(caso.tipoPoliza || '').trim();
  const detalle = String(caso.tipoPolizaOtro || '').trim();
  if (esTipoPolizaOtroPrevisora(tipo) && detalle) return detalle;
  return tipo;
};

/** Tomadores base del consolidado Previsora (columna TOMADOR). */
export const TOMADORES_PREVISORA_DEFAULT = [
  'BANCO AV VILLAS',
  'BANCO BOGOTA',
  'BANCO OCCIDENTE',
  'BANCO POPULAR',
...
export const ETIQUETAS_ARCHIVO_PREVISORA = [
  'GENERAL',
  'POLIZA',
  'INSPECCION',
  'LIQUIDACION',
...
/** Manual CAT Previsora: severidad 1–6 (reporte de exposición). Textos exactos del Word. */
export const SEVERIDAD_CAT_PREVISORA = [
...
export const ACCESO_PREDIO_PREVISORA = ['SI', 'NO', 'PARCIAL'];
...
export const DISCLAIMER_CAT_PREVISORA =
  'Documento operativo para clasificar severidad, completar la base de exposición y registrar evidencia. No autoriza confirmar ni negar cobertura, prometer pagos ni actuar como vocero de Previsora. La severidad es preliminar y no constituye liquidación del siniestro.';

export const labelSeveridadCat = (nivel) => {
  const n = Number(nivel);
  const found = SEVERIDAD_CAT_PREVISORA.find((s) => s.valor === n);
...
export const OPCIONES_SI_NO_PREVISORA = ['SI', 'NO'];

/** Grado de afectación (1–6), independiente de la severidad CAT del desprendible */
export const GRADOS_AFECTACION_PREVISORA = ['1', '2', '3', '4', '5', '6'];

export const normalizarSiNoPrevisora = (raw) => {
...
export const normalizarGradoAfectacionPrevisora = (raw) => {
...
export const CAMPOS_FECHA_Previsora = [
  'fechaSiniestro',
  'fechaInicioPoliza',
  'fechaFinPoliza',
  'fechaInspeccion',
...
export const CAMPOS_DECIMAL_PREVISORA = ['distanciaEpicentroKm'];
...
export const construirFormDesdecasoPrevisora = (caso = {}) => {
    ...
    ...Object.fromEntries(
      Object.keys(FORM_VACIO_PREVISORA)
        ...
        .map((clave) => {
          ...
          if (CAMPOS_FECHA_Previsora.includes(clave)) return [clave, fechaParaInput(valor)];
          if (CAMPOS_NUMERICOS_PREVISORA.includes(clave)) return [clave, formatMiles(valor)];
          if (CAMPOS_DECIMAL_PREVISORA.includes(clave)) {
          ...
          if (clave === 'afectacion' || clave === 'lucroCesante') {
            return [clave, normalizarSiNoPrevisora(valor)];
          }
          if (clave === 'gradoAfectacion') {
            return [clave, normalizarGradoAfectacionPrevisora(valor)];
          }
  ...
  if (base.tipoPoliza && !TIPOS_POLIZA_PREVISORA.includes(base.tipoPoliza)) {
