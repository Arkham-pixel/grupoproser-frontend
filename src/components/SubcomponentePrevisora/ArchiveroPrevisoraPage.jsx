...
import { fetchAllCasosPrevisora, getCasoPrevisoraById } from '../../services/previsoraService.js';
...
export default function ArchiveroPrevisoraPage() {
  ...
  useEffect(() => {
    ...
    async function cargar() {
      ...
      setError('');
      try {
        const actual = await getCasoPrevisoraById(casoIdFromQuery);
  ...
  const subtitulo = useMemo(() => {
    ...
    return t('previsora.archive.pageSubtitle');
  }, [caso, t]);
      ...
      <div className={expressPageWrap}>
        ...
        {!casoId && !cargandoCaso && (
          <PrevisoraCasoPicker
            ...
            onSelect={elegirCaso}
            hint={t('previsora.archive.pickCase')}
          />
        )}
        ...
        <div className={expressCard}>
          <div className={expressCardBody}>
            {cargandoCaso ? (
              ...
              <ArchiveroPrevisora caso={caso} onChanged={setCaso} />
            ) : (
              <p className="text-sm text-gray-500">{t('previsora.archive.pickCase')}</p>
            )}
          </div>
        </div>
      </div>
