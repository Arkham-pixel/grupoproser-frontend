...
import { fetchAllCasosPrevisora, getAlertasPrevisora } from '../../services/previsoraService.js';
import { formatCurrency } from './previsoraHelpers.js';
import {
  calcularBoletinSemanalPrevisora,
  inicioSemanaBogota,
  shiftSemana,
  sumarDiasUtc,
} from './boletinSemanalPrevisoraHelpers.js';
import {
  expressBtnGhost,
  expressBtnPrimary,
  expressCard,
...
const NOTES_KEY = 'previsora.boletinNotas';
...
export default function BoletinSemanalprevisora() {
    ...
    (async () => {
      ...
      try {
        const [lista, alertas] = await Promise.all([
          ...
          getAlertasPrevisora().catch(() => null),
        ]);
        if (!cancelado) {
          setCasos(lista);
        ...
        console.error('Error cargando boletín Previsora:', err);
        if (!cancelado) {
          setError(err.message || t('previsora.boletin.loadError'));
          setCasos([]);
        }
      } finally {
  ...
  const boletin = useMemo(
    () => calcularBoletinSemanalPrevisora(casos, alertasPayload, { desde, hasta }),
      ...
      <div className={expressPageWrap}>
        <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between print:block">
          <div>
            ...
            <h1 className={expressPageTitle}>{t('previsora.boletin.title')}</h1>
            <p className={expressPageSubtitle}>
              {t('previsora.boletin.subtitle', { rango: boletin.rango.etiqueta })}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2 print:hidden">
            <button
              ...
              aria-label={t('previsora.boletin.prevWeek')}
            >
              <FaChevronLeft /> {t('previsora.boletin.prevWeek')}
            </button>
            <button
              type="button"
              className={expressBtnGhost}
            ...
            <button
              ...
              aria-label={t('previsora.boletin.nextWeek')}
            >
              {t('previsora.boletin.nextWeek')} <FaChevronRight />
            </button>
            <button type="button" className={expressBtnPrimary} onClick={() => window.print()}>
              <FaPrint /> {t('previsora.boletin.print')}
            </button>
            <Link to="/previsora/dashboard" className={expressBtnGhost}>
              {t('nav.previsoraDashboard')}
            </Link>
            <Link to="/previsora/reporte" className={expressBtnGhost}>
              {t('nav.previsoraReport')}
            </Link>
          </div>
        </header>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <ExpressMetricCard
            label={t('previsora.boletin.kpis.reported')}
            value={String(boletin.kpis.casosReportados)}
            hint={t('previsora.boletin.kpis.reportedHint')}
          />
          <ExpressMetricCard
            label={t('previsora.boletin.kpis.inspected')}
            value={String(boletin.kpis.casosInspeccionados)}
            hint={t('previsora.boletin.kpis.inspectedHint')}
          />
          <ExpressMetricCard
            label={t('previsora.boletin.kpis.settled')}
            value={String(boletin.kpis.casosLiquidados)}
            hint={t('previsora.boletin.kpis.settledHint')}
          />
          <ExpressMetricCard
            label={t('previsora.boletin.kpis.active')}
            value={`${boletin.kpis.casosActivos} / ${boletin.kpis.totalCasos}`}
            hint={t('previsora.boletin.kpis.activeHint')}
          />
        </section>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <ExpressMetricCard
            label={t('previsora.boletin.kpis.reserve')}
            value={formatCurrency(boletin.kpis.reservaEstimada)}
            hint={t('previsora.boletin.kpis.reserveHint')}
          />
          <ExpressMetricCard
            label={t('previsora.boletin.kpis.claimedActive')}
            value={formatCurrency(boletin.kpis.valorReclamadoActivos)}
            hint={t('previsora.boletin.kpis.claimedActiveHint')}
          />
          <ExpressMetricCard
            label={t('previsora.boletin.kpis.claimedWeek')}
            value={formatCurrency(boletin.kpis.valorReclamadoSemana)}
            hint={t('previsora.boletin.kpis.claimedWeekHint')}
          />
          <ExpressMetricCard
            label={t('previsora.boletin.kpis.adjustedWeek')}
            value={formatCurrency(boletin.kpis.valorAjustadoSemana)}
            hint={t('previsora.boletin.kpis.adjustedWeekHint')}
          />
        </section>
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          <div className={expressCard}>
            ...
            <div className={`${expressCardBody} grid gap-4 sm:grid-cols-2`}>
              ...
              <div className="rounded-xl border border-gray-100 p-4 dark:border-gray-800">
                <p className="text-sm font-medium text-gray-500 dark:text-gray-400">
                  {t('previsora.boletin.ans.settlement')}
                </p>
                <p className="mt-2 font-accent text-2xl font-bold text-gray-900 dark:text-white">
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          <div className={expressCard}>
            <div className={expressCardHeader}>
              <h2 className="font-heading text-lg font-bold text-gray-900 dark:text-white">
                {t('previsora.boletin.news.title')}
              </h2>
            </div>
            <div className={expressCardBody}>
              {boletin.novedades.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {t('previsora.boletin.news.empty')}
                </p>
              ) : (
              ...
              <label className="mt-4 block print:hidden">
                <span className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-gray-300">
                  {t('previsora.boletin.news.extra')}
                </span>
                <textarea
                  ...
                  placeholder={t('previsora.boletin.news.extraPlaceholder')}
                />
              </label>
          ...
          <div className={expressCard}>
            <div className={expressCardHeader}>
              <h2 className="font-heading text-lg font-bold text-gray-900 dark:text-white">
                {t('previsora.boletin.risks.title')}
              </h2>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                {t('previsora.boletin.risks.summary', {
            ...
            <div className={expressCardBody}>
              {boletin.alertas.items.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {t('previsora.boletin.risks.empty')}
                </p>
              ) : (
                <ul className="space-y-2">
              ...
              <label className="mt-4 block print:hidden">
                <span className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-gray-300">
                  {t('previsora.boletin.risks.support')}
                </span>
                <textarea
                  className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-gray-700 dark:bg-[#1A1A1A] dark:text-gray-200"
                  rows={3}
                  ...
                  placeholder={t('previsora.boletin.risks.supportPlaceholder')}
                />
              </label>
              {notas.apoyo && (
                <p className="mt-3 whitespace-pre-wrap text-sm text-gray-700 dark:text-gray-300">
                  <strong>{t('previsora.boletin.risks.support')}:</strong> {notas.apoyo}
                </p>
              )}
            </div>
          </div>
        ...
        <section className={expressCard}>
          <div className={expressCardHeader}>
            <h2 className="font-heading text-lg font-bold text-gray-900 dark:text-white">
              {t('previsora.boletin.detail.title')}
            </h2>
          </div>
            ...
            <table className="min-w-full text-left text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-xs uppercase text-gray-500 dark:border-gray-800">
                  <th className="px-2 py-2">{t('previsora.boletin.detail.type')}</th>
                  <th className="px-2 py-2">{t('previsora.fields.siniestro')}</th>
                  <th className="px-2 py-2">{t('previsora.fields.asegurado')}</th>
                  <th className="px-2 py-2">{t('previsora.fields.estado')}</th>
                  <th className="px-2 py-2">{t('previsora.fields.ciudad')}</th>
                </tr>
              </thead>
              <tbody>
                {filasDetalle.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-2 py-4 text-gray-500">
                      {t('previsora.boletin.detail.empty')}
                    </td>
                  </tr>
                ) : (
