...
const SEVERIDAD_MANUAL_CAT = SEVERIDAD_CAT_PREVISORA.map((s) => ({
  nivel: s.valor,
...
export default function InspeccionCatPrevisora({ casoPrevisora = null, onCasoChange }) {
  ...
  const [archivos, setArchivos] = useState(() => casoPrevisora?.archivos || []);
  const [observacionesCat, setObservacionesCat] = useState(
    () => casoPrevisora?.observacionesCat || ''
  );

  const [severidadNiveles, setSeveridadNiveles] = useState(() =>
    normalizeSeveridadCatNiveles(casoPrevisora?.severidadCatNiveles, casoPrevisora?.severidadCat)
  );
  ...
  useEffect(() => {
    ...
    setArchivos(casoPrevisora?.archivos || []);
  ...
  const persistirOrdenFotos = async (fotosOrdenadas) => {
    ...
    if (!casoPrevisora?._id || !ids.length) return;
    try {
      await reordenarArchivosPrevisora(casoPrevisora._id, ids);
    } catch (err) {
  ...
  const actualizarDescripcionFoto = (fotoId, descripcion) => {
    ...
    descripcionTimeoutRef.current[clave] = setTimeout(async () => {
      if (!casoPrevisora?._id || !clave || clave.startsWith('tmp-')) return;
      try {
        await actualizarArchivoPrevisora(casoPrevisora._id, clave, { descripcion });
      } catch (err) {
  ...
  const subirFotos = async (fileList) => {
    ...
    if (!casoPrevisora?._id) {
      setError(t('previsora.cat.needSavedCase'));
      return;
    }
    setError('');
    setMensaje('');
    ...
    try {
      ...
      for (const original of files) {
        ...
        const creado = await subirArchivoPrevisora(casoPrevisora._id, file, 'FOTOS', {
          descripcion: '',
        });
      ...
      try {
        ...
        console.warn('Refresh archivos Previsora:', refreshErr);
      }
      setMensaje(
        files.length === 1
          ? t('previsora.archive.uploadOk')
  ...
  const buildCatPayload = () => {
    ...
    const fechaVisita =
      formatDateIso(casoPrevisora?.fechaInspeccion) || formatDateIso(new Date());
    const cat = {
      severidadCat,
  ...
  const persistirCat = async ({ silencioso = false } = {}) => {
    if (!casoPrevisora?._id) {
      if (!silencioso) setError(t('previsora.cat.needSavedCase'));
      return null;
    }
    if (!silencioso) {
      setGuardando(true);
    ...
    try {
      ...
      const actualizado = await guardarCatEnCasoPrevisora({
      ...
      onCasoChange?.(actualizado);
      if (!silencioso) setMensaje(t('previsora.cat.savedOk'));
      return actualizado;
    } catch (err) {
      if (!silencioso) setError(err.message || t('previsora.cat.savedError'));
      return null;
    } finally {
  ...
  useEffect(() => {
    if (!casoPrevisora?._id) return undefined;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [severidadNiveles, observacionesCat, evidenciaCat, casoPrevisora?._id]);
  ...
  const guardarYDesprendible = async () => {
    ...
    try {
      const { cat } = buildCatPayload();
      const guardado = await guardarCatEnCasoPrevisora({
        casoId: casoPrevisora._id,
        cat,
        casoBase: casoPrevisora,
      });
  ...
  const abrirFotoGrande = async (e, foto, urlFallback) => {
    ...
    url = url || urlFallback || urlDescargaArchivoPrevisora(foto?.ruta);
    if (!url) return;
    setImagenAmpliada({
      url,
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4 border-b border-gray-200 pb-4 dark:border-gray-700">
        <div className="space-y-1">
          <p className="font-heading text-sm font-bold tracking-widest text-[#002060]">PREVISORA</p>
      ...
      {!casoPrevisora?._id ? (
        <div className={expressAlertError}>{t('previsora.cat.needSavedCase')}</div>
      ) : null}
      ...
      <section>
        ...
        <label className="mb-1 block font-body text-sm font-semibold text-gray-800 dark:text-gray-200">
          {t('previsora.fields.observacionesCat')}
        </label>
        <textarea
        ...
        <div
          ...
          onClick={() => !subiendo && casoPrevisora?._id && inputRef.current?.click()}
          onDragEnter={(e) => {
            e.preventDefault();
            e.stopPropagation();
            if (casoPrevisora?._id && !subiendo) setDragOver(true);
          }}
          onDragOver={(e) => {
            e.preventDefault();
            e.stopPropagation();
          ...
          } ${!casoPrevisora?._id || subiendo ? 'pointer-events-none opacity-60' : ''}`}
        >
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          onClick={guardarCat}
          disabled={guardando || generandoDoc || !casoPrevisora?._id}
        >
          <FaSave className="mr-2 inline" />
          {guardando ? t('previsora.actions.saving') : t('previsora.cat.saveCat')}
        </button>
        <button
          type="button"
