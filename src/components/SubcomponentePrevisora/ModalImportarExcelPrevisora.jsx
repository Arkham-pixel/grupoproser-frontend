...
import { importarCasosPrevisora } from '../../services/previsoraService.js';
import { importarCasosPrevisoraListado } from '../../services/previsoraListadoService.js';
import {
  parsearCasosPrevisoraDesdeExcel,
  parsearListadoClientePrevisoraDesdeExcel,
} from './importarPrevisoraExcel.js';
import {
  expressBtnGhost,
  expressBtnPrimary,
} from '../SubcomponenteExpress/expressFenixUi.js';

export function esAdminOSoportePrevisora() {
...
export default function ModalImportarExcelPrevisora({ open, onClose, onCompleted }) {
  ...
  const handleFile = async (selected) => {
    ...
    try {
      ...
      let cat = null;
      try {
        listado = await parsearListadoClientePrevisoraDesdeExcel(selected);
      } catch {
        listado = null;
      }
      try {
        cat = await parsearCasosPrevisoraDesdeExcel(selected);
      } catch {
        cat = null;
      }
  ...
  const handleImport = async () => {
    ...
    try {
      const data =
        modo === 'cat'
          ? await importarCasosPrevisora(casos)
          : await importarCasosPrevisoraListado(casos);
      const totals = {
