...
const ASEGURADORA = 'Previsora S.A.';
...
async function buildLogosHeader() {
  ...
  const right = new Paragraph({
    ...
    children: Previsora
      ...
      : [new TextRun({ text: 'Previsora', bold: true, font: 'Arial', size: 18 })],
  });
...
/**
 * Finiquito / Constancia de indemnización Previsora
 * (mismo esquema legal que Fundación de la Mujer / Equidad FDM),
 * con nombres de Previsora y logos Proser + Previsora.
 */
export async function descargarFiniquitoPrevisoraWord(liquidador = {}, totalesInput) {
  const enc = liquidador.encabezado || {};
  const totales = totalesInput || calcularLiquidacionPrevisora(liquidador);
