...
const COLUMNAS = [
  ...
  { clave: 'fechaCoordinandoInspeccion', labelKey: 'fechaProgramandoInspeccion' },
...
const buildExportRow = (caso) => ({
  ...
  'FECHA PROGRAMANDO INSPECCIÓN': formatDate(caso.fechaCoordinandoInspeccion),
  'FECHA CASO INSPECCIONADO': formatDate(caso.fechaCasoInspeccionado),
...
export default function ReportePrevisora() {
  ...
  const recargar = useCallback(async ({ silencioso = false } = {}) => {
    ...
    try {
      const data = await fetchAllCasosPrevisora(2000, { soloChecklistLleno });
  ...
  const exportarExcel = () => {
    ...
    try {
      ...
      XLSX.writeFile(wb, `previsora-cat-${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (err) {
      setAviso({
        tipo: 'error',
        titulo: t('previsora.report.exportError'),
        mensaje: err.message || t('previsora.report.exportErrorMessage'),
      });
    }
  };
  ...
  const solicitarEliminar = (item) => {
      setAviso({
        ...
        onConfirm: async () => {
          try {
            await deleteCasoPrevisora(item._id);
            setAviso({
              tipo: 'success',
      ...
      <div className={wrap}>
        <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-3">
            <span className={expressBadge}>Previsora · CAT</span>
            <div>
              <h1 className={expressPageTitle}>{t('previsora.report.title')}</h1>
              <p className={expressPageSubtitle}>
                {soloChecklistLleno
                  ? t('previsora.report.subtitleChecklist')
                  : t('previsora.report.subtitle')}
              </p>
            </div>
            <nav className="flex flex-wrap gap-2">
              <Link
                to="/previsora/dashboard"
              ...
              <tbody className="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-[#1A1A1A]">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={COLUMNAS.length + 1} className="px-4 py-8 text-center text-sm text-gray-500">
                      {soloChecklistLleno
                        ? t('previsora.report.noCasesChecklist')
                    ...
                    <tr key={item._id} className="transition hover:bg-gray-50/80 dark:hover:bg-gray-900/30">
                      <td className="sticky left-0 z-10 whitespace-nowrap bg-white px-4 py-3 dark:bg-[#1A1A1A]">
                        <AccionesPrevisoraMenu
                          ...
                          onAbrirCaso={() =>
                            navigate(`/previsora/caso?casoId=${item._id}&tab=cat`, {
                              state: { casoPrevisora: item },
                            })
                          }
                          onLiquidador={() =>
                            navigate(`/previsora/liquidador?casoId=${item._id}&tab=liquidador`, {
                              state: { casoPrevisora: item },
                            })
                          }
                          onInformeUnico={() =>
                            navigate(`/previsora/informe-unico?casoId=${item._id}&tab=informe`, {
                              state: { casoPrevisora: item },
                            })
                          }
