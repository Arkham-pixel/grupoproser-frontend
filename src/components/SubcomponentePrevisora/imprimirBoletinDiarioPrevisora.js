...
/**
 * @param {object} boletin - resultado de calcularBoletinDiarioPrevisora
 * @param {object} labels - textos ya resueltos (i18n)
 */
export function construirHtmlBoletinDiarioPrevisora(boletin, labels = {}) {
...
<head>
  <meta charset="utf-8" />
  <title>${esc(L.docTitle || 'Boletín diario Previsora')}</title>
  <style>${estilosImpresion()}</style>
</head>
<body>
  <p class="eyebrow">PREVISORA</p>
 ...
 */
export function imprimirBoletinDiarioPrevisora(boletin, labels = {}) {
  const html = construirHtmlBoletinDiarioPrevisora(boletin, labels);
