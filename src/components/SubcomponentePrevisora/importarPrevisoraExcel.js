...
/** Mapeo de encabezados hoja BD / PENDIENTES / CAT_PREVISORA → campos del caso. */
const HEADER_MAP = {
  SINIESTRO: 'siniestro',
  ...
  'SOLICITUD ANTICIPO': 'solicitudAnticipo',
  ANTICIPO: 'solicitudAnticipo',
  'VALOR SOLICITUD ANTICIPO': 'valorSolicitudAnticipo',
  'VALOR ANTICIPO': 'valorSolicitudAnticipo',
  RESERVA: 'reserva',
...
const parseFechaCelda = (valor) => {
  ...
  if (mdy) {
    ...
    let month = Number(a);
    let day = Number(b);
    // Si el primero > 12, es dd/mm; si no, en consolidado Previsora suele ser m/d/yy (Excel US)
    if (month > 12 && day <= 12) {
      month = Number(b);
      day = Number(a);
    }
...
const elegirHojasCandidatas = (workbook) => {
  ...
  const basePrevisora = nombres.find((n) => /BASE Previsora/i.test(n));
  if (basePrevisora) orden.push(basePrevisora);
  if (bd) orden.push(bd);
 ...
 */
export const parsearCasosPrevisoraDesdeExcel = async (file) => {
  ...
  if (/BASE Previsora/i.test(hojaUsada)) {
    casos = casos.map((caso) => ({
      ...caso,
      causa: caso.causa || 'TERREMOTO',
      canalRadicacion: caso.canalRadicacion || 'Previsora',
      tomador: caso.tomador || 'Previsora SEGUROS',
    }));
  }
