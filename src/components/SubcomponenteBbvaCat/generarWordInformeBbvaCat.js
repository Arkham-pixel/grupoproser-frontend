 ...
 * Logo Proser | Título + subtítulo + código/versión/fecha | Logo BbvaCat
 */
async function crearEncabezadoBbvaCat({ caso = {}, informe = {} } = {}) {
        ...
        rows: [
          new TableRow({
            children: [
              ...
              logoCell(BbvaCat, 'BBVA CAT', AlignmentType.RIGHT),
            ],
          }),
        ],
      }),
...
async function construirBloqueDaniosUbicacionBbvaCat({ info = {}, caso = {} } = {}) {
  const bloques = [];
...
/**
 * Zona de firmas BbvaCat: solo ajustador, sin tabla/bordes, abajo a la izquierda.
 */
async function construirZonaFirmasBbvaCat({ info = {} } = {}) {
...
export async function descargarWordInformeBbvaCat({ caso = {}, informe = null, liquidador = null } = {}) {
  ...
  for (const archivo of fotosArchivos.slice(0, 12)) {
    const url = urlDescargaArchivoBbvaCat(archivo.ruta);
    if (!url) continue;
  ...
  const firmasParrafos = await construirZonaFirmasBbvaCat({ caso, enc, info });
  const bloqueDaniosUbicacion = await construirBloqueDaniosUbicacionBbvaCat({ info, caso });
  ...
  const nombre = `Informe_Unico_BBVA_CAT_${caso.siniestro || caso.consecutivo || 'caso'}.docx`.replace(
    /[^\w.\-áéíóúÁÉÍÓÚñÑ]+/gi,
    '_'
  );
  saveAs(blob, nombre);
