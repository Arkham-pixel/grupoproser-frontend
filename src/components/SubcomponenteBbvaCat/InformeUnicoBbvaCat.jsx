...
export default function InformeUnicoBbvaCat({
  ...
  const refrescarCaso = async () => {
    if (!casoBbvaCat?._id) return null;
  ...
  const subirFotos = async (fileList) => {
    ...
    if (!files.length) {
      setError(t('bbvaCat.reportUnique.photosOnlyImages'));
      return;
    }
    ...
    try {
      for (const file of files) {
        await subirArchivoBbvaCat(casoBbvaCat._id, file, 'FOTOS');
      }
      await refrescarCaso();
      setMensaje(t('bbvaCat.reportUnique.photosUploaded', { count: files.length }));
    } catch (err) {
      console.error(err);
      setError(err.message || t('bbvaCat.reportUnique.photosUploadError'));
    } finally {
  ...
  const handleEliminarFoto = async (archivoId) => {
    if (!casoBbvaCat?._id || !archivoId) return;
  ...
  const fotos = (Array.isArray(casoBbvaCat?.archivos) ? casoBbvaCat.archivos : []).filter((a) => {
      ...
      <section className={expressFormSection}>
        ...
        >
          ...
          <p className="font-body text-sm font-semibold text-gray-800 dark:text-gray-100">
            {subiendoFotos
              ? t('bbvaCat.reportUnique.photosUploading')
              : t('bbvaCat.reportUnique.photosDropTitle')}
          </p>
          <p className="mt-1 font-body text-xs text-gray-500">
            {t('bbvaCat.reportUnique.photosDropSubtitle')}
          </p>
        </div>

        {fotos.length === 0 ? (
          <p className="text-sm text-gray-500">{t('bbvaCat.reportUnique.noPhotos')}</p>
        ) : (
