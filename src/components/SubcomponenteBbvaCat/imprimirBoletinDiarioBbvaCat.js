...
/**
 * @param {object} boletin - resultado de calcularBoletinDiarioBbvaCat
 * @param {object} labels - textos ya resueltos (i18n)
 */
export function construirHtmlBoletinDiarioBbvaCat(boletin, labels = {}) {
...
<head>
  <meta charset="utf-8" />
  <title>${esc(L.docTitle || 'Boletín diario BBVA CAT')}</title>
  <style>${estilosImpresion()}</style>
</head>
<body>
  <p class="eyebrow">BBVA CAT</p>
 ...
 */
export function imprimirBoletinDiarioBbvaCat(boletin, labels = {}) {
  const html = construirHtmlBoletinDiarioBbvaCat(boletin, labels);
