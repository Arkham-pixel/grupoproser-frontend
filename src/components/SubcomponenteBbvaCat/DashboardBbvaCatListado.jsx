...
export default function DashboardBbvaCatListado() {
  ...
  const td = (key, opts) => t(`bbvaCat.listadoDashboard.${key}`, opts);
  ...
  const casosFiltrados = useMemo(
    () =>
      casos.filter((item) => {
        if (filtroCiudad && !coincideFiltroCiudadBbvaCat(item.ciudad, filtroCiudad)) return false;
        if (filtroEstado && !coincideFiltroTexto(item.estado, filtroEstado)) return false;
        if (filtroTipoPoliza && !coincideFiltroTexto(etiquetaTipoPolizaBbvaCat(item), filtroTipoPoliza)) {
          return false;
        }
        ...
        if (fechaDesde || fechaHasta) {
          return fechaEnRango(fechaAltaListadoBbvaCat(item), fechaDesde, fechaHasta);
        }
        return true;
      }),
    [
      ...
      <div className={`${expressPageWrap} min-w-0`}>
        ...
        <section className="grid w-full min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          ...
          <ExpressMetricCard
            ...
            hint={td('kpis.objectionHint', { closed: kpis.objetados })}
          />
          <ExpressMetricCard
        ...
        <section className="grid w-full min-w-0 grid-cols-1 items-stretch gap-4 lg:grid-cols-2">
          ...
          <HorizontalBars
            ...
            gridStroke={gridStroke}
            tooltipStyle={tooltipStyle}
            seriesName={t('bbvaCat.fields.tipoPoliza')}
          />
        </section>
        ...
        <section className="grid w-full min-w-0 grid-cols-1 items-stretch gap-4 lg:grid-cols-2">
          <HorizontalBars
            ...
            gridStroke={gridStroke}
            tooltipStyle={tooltipStyle}
            seriesName={t('bbvaCat.fields.causa')}
          />
          <HorizontalBars
            ...
            gridStroke={gridStroke}
            tooltipStyle={tooltipStyle}
            seriesName={t('bbvaCat.fields.intermediario')}
          />
        </section>
...
function ChartCard({ title, hint, empty, children }) {
  ...
  return (
    <div className={`${expressChartCard} min-w-0`}>
      ...
      {empty ? (
        <p className="font-body text-sm text-gray-500 dark:text-gray-400">{t('bbvaCat.listadoDashboard.noData')}</p>
      ) : (
        children
      )}
    </div>
