/**
 * Helpers del boletín diario BBVA CAT (corte día a día, America/Bogota).
 * Independiente del boletín semanal.
 */

import { homologarEstadoBbvaCat } from './bbvaCatHelpers.js';
import {
  isoDateBogota,
  parseFechaCaso,
  sumarDiasUtc,
...
export function casoIdBbvaCat(caso) {
...
export function clasificarCasoGestionTerremoto(caso = {}) {
  ...
  if (estado === 'CASO AJUSTADO' || estado === 'CASO PARA PAGO' || estado === 'PAGADO')
...
export function fechaIngresoCasoBbvaCat(caso = {}) {
  return (
...
export function casoExistiaAlCorte(caso, isoCorte) {
  const ingreso = fechaIngresoCasoBbvaCat(caso);
...
export function calcularBoletinDiarioBbvaCat(casos = [], { fechaCorte = new Date(), persistirCorte = true } = {}) {
