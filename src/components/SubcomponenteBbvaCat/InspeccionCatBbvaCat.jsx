...
import logoBbvaCat from '../../assets/zurich-logo.png';
import {
  actualizarArchivoBbvaCat,
  eliminarArchivoBbvaCat,
  getCasoBbvaCatById,
  guardarCatEnCasoBbvaCat,
...
const OBJETIVO_MANUAL =
  'Objetivo: entregar al ajustador información clave, sencilla y resumida para clasificar la severidad, completar la base de Excel y registrar evidencia fotográfica/documental. Este documento no autoriza a confirmar cobertura, negar cobertura, prometer pagos o actuar como vocero de BBVA.';
...
export default function InspeccionCatBbvaCat({ casoBbvaCat = null, onCasoChange }) {
  ...
  const [archivos, setArchivos] = useState(() => casoBbvaCat?.archivos || []);
  const [observacionesCat, setObservacionesCat] = useState(
    () => casoBbvaCat?.observacionesCat || ''
  );

  const [severidadNiveles, setSeveridadNiveles] = useState(() =>
    normalizeSeveridadCatNiveles(casoBbvaCat?.severidadCatNiveles, casoBbvaCat?.severidadCat)
  );
  ...
  useEffect(() => {
    ...
    setArchivos(casoBbvaCat?.archivos || []);
  ...
  const refrescarArchivos = async () => {
    if (!casoBbvaCat?._id) return;
    const actualizado = await getCasoBbvaCatById(casoBbvaCat._id);
  ...
  const persistirOrdenFotos = async (fotosOrdenadas) => {
    ...
    if (!casoBbvaCat?._id || !ids.length) return;
    try {
      await reordenarArchivosBbvaCat(casoBbvaCat._id, ids);
    } catch (err) {
  ...
  const actualizarDescripcionFoto = (fotoId, descripcion) => {
    ...
    descripcionTimeoutRef.current[clave] = setTimeout(async () => {
      if (!casoBbvaCat?._id || !clave || clave.startsWith('tmp-')) return;
      try {
        await actualizarArchivoBbvaCat(casoBbvaCat._id, clave, { descripcion });
      } catch (err) {
  ...
  const subirFotos = async (fileList) => {
    ...
    if (!casoBbvaCat?._id) {
      setError(t('bbvaCat.cat.needSavedCase'));
      return;
    }
    setError('');
    setMensaje('');
    ...
    try {
      ...
      try {
        ...
        console.warn('Refresh archivos BbvaCat:', refreshErr);
      }
      setMensaje(
        files.length === 1
  ...
  const buildCatPayload = () => {
    ...
    const fechaVisita =
      formatDateIso(casoBbvaCat?.fechaInspeccion) || formatDateIso(new Date());
    const cat = {
      severidadCat,
  ...
  const persistirCat = async ({ silencioso = false } = {}) => {
    ...
    try {
      ...
      const actualizado = await guardarCatEnCasoBbvaCat({
      ...
      onCasoChange?.(actualizado);
      if (!silencioso) setMensaje(t('bbvaCat.cat.savedOk'));
      return actualizado;
    } catch (err) {
      if (!silencioso) setError(err.message || t('bbvaCat.cat.savedError'));
      return null;
    } finally {
  ...
  useEffect(() => {
    if (!casoBbvaCat?._id) return undefined;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [severidadNiveles, observacionesCat, evidenciaCat, casoBbvaCat?._id]);
  ...
  const guardarYDesprendible = async () => {
    ...
    try {
      const { cat } = buildCatPayload();
      const guardado = await guardarCatEnCasoBbvaCat({
        casoId: casoBbvaCat._id,
        cat,
        casoBase: casoBbvaCat,
      });
      ...
      const base = {
        ...(guardado || {}),
        ...cat,
        archivos: archivos?.length ? archivos : guardado?.archivos || casoBbvaCat.archivos || [],
      };
      const nombre = await descargarDesprendibleCatBbvaCat(base);
  ...
  const exportarDesprendibleSinGuardar = async () => {
    ...
    try {
      ...
      const base = {
        ...(casoBbvaCat || {}),
        ...cat,
        archivos: archivos?.length ? archivos : casoBbvaCat?.archivos || [],
      };
      const nombre = await descargarDesprendibleCatBbvaCat(base);
  ...
  const abrirFotoGrande = async (e, foto, urlFallback) => {
    ...
    url = url || urlFallback || urlDescargaArchivoBbvaCat(foto?.ruta);
    if (!url) return;
    setImagenAmpliada({
      url,
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4 border-b border-gray-200 pb-4 dark:border-gray-700">
        <div className="space-y-1">
          <p className="font-heading text-sm font-bold tracking-widest text-[#002060]">BBVA_CAT</p>
      ...
      {!casoBbvaCat?._id ? (
        <div className={expressAlertError}>{t('bbvaCat.cat.needSavedCase')}</div>
      ) : null}
      ...
      <section>
        ...
        <label className="mb-1 block font-body text-sm font-semibold text-gray-800 dark:text-gray-200">
          {t('bbvaCat.fields.observacionesCat')}
        </label>
        <textarea
        ...
        <div
          ...
          onClick={() => !subiendo && casoBbvaCat?._id && inputRef.current?.click()}
          onDragEnter={(e) => {
            e.preventDefault();
            e.stopPropagation();
            if (casoBbvaCat?._id && !subiendo) setDragOver(true);
          }}
          onDragOver={(e) => {
            e.preventDefault();
            e.stopPropagation();
          ...
          } ${!casoBbvaCat?._id || subiendo ? 'pointer-events-none opacity-60' : ''}`}
        >
