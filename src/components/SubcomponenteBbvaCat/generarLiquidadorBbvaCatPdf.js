 ...
 */
export async function generarLiquidadorBbvaCatPdfBlob(liquidador) {
  const evalData = liquidador?.evaluacionSismicaNSR10 || {};
  const enc = liquidador?.encabezado || {};
  ...
  doc.setProperties({
    title: `Evaluación Sísmica NSR-10 BbvaCat - ${enc.siniestro || enc.asegurado || ''}`,
...
export async function descargarLiquidadorBbvaCatPdf(liquidador, totales) {
  ...
  const safe = String(enc.siniestro || enc.consecutivo || 'NSR10')
    .replace(/[^\w.-]+/g, '_')
    .slice(0, 40);
  saveAs(blob, `Evaluacion_Sismica_NSR10_BbvaCat_${safe}.pdf`);
}
