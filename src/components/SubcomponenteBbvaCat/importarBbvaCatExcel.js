...
import {
  ...
  normalizarGradoAfectacionBbvaCat,
  normalizarSiNoBbvaCat,
...
/** Mapeo de encabezados hoja BD / PENDIENTES / CAT_BBVA → campos del caso. */
const HEADER_MAP = {
  SINIESTRO: 'siniestro',
...
const parseFechaCelda = (valor) => {
  ...
  if (mdy) {
    ...
    let month = Number(a);
    let day = Number(b);
    // Si el primero > 12, es dd/mm; si no, en consolidado BbvaCat suele ser m/d/yy (Excel US)
    if (month > 12 && day <= 12) {
      month = Number(b);
      day = Number(a);
    }
...
const parsearHojaACasos = (sheet) => {
  ...
  for (let r = headerRowIdx + 1; r < matriz.length; r += 1) {
    ...
    caso.estado = homologarEstadoBbvaCat(caso.estado);
...
const elegirHojasCandidatas = (workbook) => {
  ...
  const baseBbva = nombres.find((n) => /BASE BBVA/i.test(n));
  if (baseBbva) orden.push(baseBbva);
  if (bd) orden.push(bd);
 ...
 */
export const parsearCasosBbvaCatDesdeExcel = async (file) => {
  ...
  if (/BASE BBVA/i.test(hojaUsada)) {
    casos = casos.map((caso) => ({
      ...caso,
      causa: caso.causa || 'TERREMOTO',
      canalRadicacion: caso.canalRadicacion || 'BBVA',
      tomador: caso.tomador || 'BBVA SEGUROS',
    }));
  }
 ...
 */
export const parsearListadoClienteBbvaCatDesdeExcel = async (file) => {
  if (!file) throw new Error('No se seleccionó archivo');
  const buffer = await file.arrayBuffer();
  const workbook = XLSX.read(buffer, { type: 'array', cellDates: true });
  if (!workbook.SheetNames?.length) throw new Error('El archivo no contiene hojas válidas');
