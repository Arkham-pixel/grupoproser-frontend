/**
 * Desprendible Manual CAT BBVA (inspección / exposición).
...
import logoBbvaCatUrl from '../../assets/zurich-logo.png';
...
async function construirParrafosFotos(caso = {}) {
  ...
  for (let i = 0; i < fotosOrdenadas.length; i += 1) {
    ...
    const urls = await candidatosUrlArchivoParaFetch(a.ruta, urlDescargaArchivoBbvaCat(a.ruta));
    let img = null;
...
/**
 * @param {object} caso — caso BBVA CAT (con severidadCatNiveles / evidenciaCat)
 */
export async function generarDesprendibleCatBbvaCat(caso = {}) {
  ...
  const logoBbvaCat = await loadLogoBytes(logoBbvaCatUrl);
          ...
          default: new Header({
            children: [
              new Paragraph({
                ...
                children: logoBbvaCat
                  ? [
                      new ImageRun({
                        data: logoBbvaCat.bytes,
                        transformation: { width: 110, height: 44 },
                        type: logoBbvaCat.type,
                      }),
                    ]
                  : [
                      new TextRun({
                ...
                children: [
                  new TableCell({
                    ...
                    children: [
                      ...
                      p(DISCLAIMER_CAT_BBVA, { size: 15, italics: true, before: 40, after: 60 }),
                    ],
                  }),
                ],
              }),
...
export async function descargarDesprendibleCatBbvaCat(caso) {
  const doc = await generarDesprendibleCatBbvaCat(caso);
  const blob = await Packer.toBlob(doc);
  const nombre = `Desprendible_CAT_BBVA_CAT_${caso.consecutivo || caso.identificacion || 'caso'}.docx`;
  saveAs(blob, nombre);
  return nombre;
}
