/**
 * Desprendible Manual CAT Allianz (inspección / exposición).
...
async function construirParrafosFotos(caso = {}) {
  ...
  for (let i = 0; i < fotosOrdenadas.length; i += 1) {
    ...
    const url = urlDescargaArchivoAllianz(a.ruta);
...
/**
 * @param {object} caso — caso Allianz (con severidadCatNiveles / evidenciaCat)
 */
export async function generarDesprendibleCatAllianz(caso = {}) {
  ...
  const logoAllianz = await loadLogoBytes(logoAllianzUrl);
                ...
                children: [
                  new TableCell({
                    ...
                    children: [
                      ...
                      p(DISCLAIMER_CAT_ALLIANZ, { size: 15, italics: true, before: 40, after: 60 }),
                    ],
                  }),
                ],
              }),
...
export async function descargarDesprendibleCatAllianz(caso) {
  const doc = await generarDesprendibleCatAllianz(caso);
  const blob = await Packer.toBlob(doc);
  const nombre = `Desprendible_CAT_ALLIANZ_${caso.consecutivo || caso.identificacion || 'caso'}.docx`;
  saveAs(blob, nombre);
  return nombre;
}
