...
import { descargarDesprendibleCatAllianz } from './generarDesprendibleCatAllianz.js';
...
const SEVERIDAD_MANUAL_CAT = SEVERIDAD_CAT_ALLIANZ.map((s) => ({
  nivel: s.valor,
  descripcion: s.descripcion,
}));

const OBJETIVO_MANUAL =
  'Objetivo: entregar al ajustador información clave, sencilla y resumida para clasificar la severidad, completar la base de Excel y registrar evidencia fotográfica/documental. Este documento no autoriza a confirmar cobertura, negar cobertura, prometer pagos o actuar como vocero de Allianz.';
 ...
 */
export default function InspeccionCatAllianz({ casoAllianz = null, onCasoChange }) {
  ...
  const [archivos, setArchivos] = useState(() => casoAllianz?.archivos || []);
  const [observacionesCat, setObservacionesCat] = useState(
    () => casoAllianz?.observacionesCat || ''
  );

  const [severidadNiveles, setSeveridadNiveles] = useState(() =>
    normalizeSeveridadCatNiveles(casoAllianz?.severidadCatNiveles, casoAllianz?.severidadCat)
  );
  ...
  useEffect(() => {
    ...
    setArchivos(casoAllianz?.archivos || []);
  ...
  const refrescarArchivos = async () => {
    if (!casoAllianz?._id) return;
    const actualizado = await getCasoAllianzById(casoAllianz._id);
  ...
  const persistirOrdenFotos = async (fotosOrdenadas) => {
    ...
    if (!casoAllianz?._id || !ids.length) return;
    try {
      await reordenarArchivosAllianz(casoAllianz._id, ids);
    } catch (err) {
  ...
  const actualizarDescripcionFoto = (fotoId, descripcion) => {
    ...
    descripcionTimeoutRef.current[clave] = setTimeout(async () => {
      if (!casoAllianz?._id || !clave || clave.startsWith('tmp-')) return;
      try {
        await actualizarArchivoAllianz(casoAllianz._id, clave, { descripcion });
      } catch (err) {
  ...
  const subirFotos = async (fileList) => {
    ...
    if (!casoAllianz?._id) {
      setError(t('allianz.cat.needSavedCase'));
      return;
    }
    setError('');
    setMensaje('');
    ...
    try {
      ...
      for (const original of files) {
        ...
        const creado = await subirArchivoAllianz(casoAllianz._id, file, 'FOTOS', {
          descripcion: '',
        });
      ...
      try {
        ...
        console.warn('Refresh archivos Allianz:', refreshErr);
      }
      setMensaje(
        files.length === 1
          ? t('allianz.archive.uploadOk')
  ...
  const handleDeleteArchivo = async (archivoId) => {
    ...
    try {
      await eliminarArchivoAllianz(casoAllianz._id, archivoId);
  ...
  const buildCatPayload = () => {
    ...
    const fechaVisita =
      formatDateIso(casoAllianz?.fechaInspeccion) || formatDateIso(new Date());
    const cat = {
      severidadCat,
  ...
  const persistirCat = async ({ silencioso = false } = {}) => {
    if (!casoAllianz?._id) {
      if (!silencioso) setError(t('allianz.cat.needSavedCase'));
      return null;
    }
    if (!silencioso) {
      setGuardando(true);
    ...
    try {
      ...
      const actualizado = await guardarCatEnCasoAllianz({
      ...
      onCasoChange?.(actualizado);
      if (!silencioso) setMensaje(t('allianz.cat.savedOk'));
      return actualizado;
    } catch (err) {
      if (!silencioso) setError(err.message || t('allianz.cat.savedError'));
      return null;
    } finally {
  ...
  useEffect(() => {
    if (!casoAllianz?._id) return undefined;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [severidadNiveles, observacionesCat, evidenciaCat, casoAllianz?._id]);
  ...
  const guardarYDesprendible = async () => {
    ...
    try {
      const { cat } = buildCatPayload();
      const guardado = await guardarCatEnCasoAllianz({
        casoId: casoAllianz._id,
        cat,
        casoBase: casoAllianz,
      });
      ...
      const base = {
        ...(guardado || {}),
        ...cat,
        archivos: archivos?.length ? archivos : guardado?.archivos || casoAllianz.archivos || [],
      };
      const nombre = await descargarDesprendibleCatAllianz(base);
  ...
  const exportarDesprendibleSinGuardar = async () => {
    ...
    try {
      ...
      const base = {
        ...(casoAllianz || {}),
        ...cat,
        archivos: archivos?.length ? archivos : casoAllianz?.archivos || [],
      };
      const nombre = await descargarDesprendibleCatAllianz(base);
  ...
  const abrirFotoGrande = async (e, foto, urlFallback) => {
    ...
    url = url || urlFallback || urlDescargaArchivoAllianz(foto?.ruta);
    if (!url) return;
    setImagenAmpliada({
      url,
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4 border-b border-gray-200 pb-4 dark:border-gray-700">
        <div className="space-y-1">
          <p className="font-heading text-sm font-bold tracking-widest text-[#002060]">ALLIANZ</p>
      ...
      {!casoAllianz?._id ? (
        <div className={expressAlertError}>{t('allianz.cat.needSavedCase')}</div>
      ) : null}
      ...
      <section>
        ...
        <label className="mb-1 block font-body text-sm font-semibold text-gray-800 dark:text-gray-200">
          {t('allianz.fields.observacionesCat')}
        </label>
        <textarea
        ...
        <div
          ...
          onClick={() => !subiendo && casoAllianz?._id && inputRef.current?.click()}
          onDragEnter={(e) => {
            e.preventDefault();
            e.stopPropagation();
            if (casoAllianz?._id && !subiendo) setDragOver(true);
          }}
          onDragOver={(e) => {
            e.preventDefault();
            e.stopPropagation();
          ...
          } ${!casoAllianz?._id || subiendo ? 'pointer-events-none opacity-60' : ''}`}
        >
        ...
        {fotos.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {fotos.map((f, index) => {
              ...
              const tieneFoto = Boolean(f.ruta);
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          onClick={guardarCat}
          disabled={guardando || generandoDoc || !casoAllianz?._id}
        >
          <FaSave className="mr-2 inline" />
          {guardando ? t('allianz.actions.saving') : t('allianz.cat.saveCat')}
        </button>
        <button
          type="button"
