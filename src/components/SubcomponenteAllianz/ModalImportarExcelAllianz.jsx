...
import { importarCasosAllianz } from '../../services/allianzService.js';
import { importarCasosAllianzListado } from '../../services/allianzListadoService.js';
import {
  parsearCasosAllianzDesdeExcel,
  parsearListadoClienteAllianzDesdeExcel,
} from './importarAllianzExcel.js';
import {
  expressBtnGhost,
  expressBtnPrimary,
} from '../SubcomponenteExpress/expressFenixUi.js';

export function esAdminOSoporteAllianz() {
...
/**
 * Carga masiva Allianz:
 * - BASE ALLIANZ / consolidado CAT → expedientes de inspección (liquidador e informe).
 * - Listado cliente (siniestro) → módulo listado.
 */
export default function ModalImportarExcelAllianz({ open, onClose, onCompleted }) {
  ...
  const handleFile = async (selected) => {
    ...
    try {
      ...
      let cat = null;
      try {
        listado = await parsearListadoClienteAllianzDesdeExcel(selected);
      } catch {
        listado = null;
      }
      try {
        cat = await parsearCasosAllianzDesdeExcel(selected);
      } catch {
        cat = null;
      }
      const esBaseAllianz = /BASE ALLIANZ/i.test(cat?.hoja || '');
      const usarCat =
        Boolean(cat?.casos?.length) &&
        (esBaseAllianz || !listado?.casos?.length || cat.casos.length >= listado.casos.length);
      const parsed = usarCat ? cat : listado;
      if (!parsed?.casos?.length) {
        throw new Error(
          'No se encontraron filas válidas. Use BASE ALLIANZ (siniestro, ID, asegurado) o el listado con siniestro, asegurado y ciudad.'
        );
      }
  ...
  const handleImport = async () => {
    ...
    try {
      const data =
        modo === 'cat'
          ? await importarCasosAllianz(casos)
          : await importarCasosAllianzListado(casos);
      const totals = {
        ...
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <p className="font-heading text-lg font-bold text-gray-900 dark:text-white">
              Importar Excel Allianz
            </p>
            <p className="mt-1 font-body text-sm text-gray-500">
              {modo === 'cat'
                ? 'Formato BASE ALLIANZ / consolidado: se crean o actualizan expedientes (liquidador e informe) por siniestro o identificación.'
