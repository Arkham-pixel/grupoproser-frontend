...
export default function InformeUnicoAllianz({
  ...
  const refrescarCaso = async () => {
    if (!casoAllianz?._id) return null;
  ...
  const subirFotos = async (fileList) => {
    ...
    if (!files.length) {
      setError(t('allianz.reportUnique.photosOnlyImages'));
      return;
    }
    ...
    try {
      for (const file of files) {
        await subirArchivoAllianz(casoAllianz._id, file, 'FOTOS');
      }
      await refrescarCaso();
      setMensaje(t('allianz.reportUnique.photosUploaded', { count: files.length }));
    } catch (err) {
      console.error(err);
      setError(err.message || t('allianz.reportUnique.photosUploadError'));
    } finally {
  ...
  const handleEliminarFoto = async (archivoId) => {
    if (!casoAllianz?._id || !archivoId) return;
  ...
  const fotos = (Array.isArray(casoAllianz?.archivos) ? casoAllianz.archivos : []).filter((a) => {
      ...
      <section className={expressFormSection}>
        ...
        >
          ...
          <p className="font-body text-sm font-semibold text-gray-800 dark:text-gray-100">
            {subiendoFotos
              ? t('allianz.reportUnique.photosUploading')
              : t('allianz.reportUnique.photosDropTitle')}
          </p>
          <p className="mt-1 font-body text-xs text-gray-500">
            {t('allianz.reportUnique.photosDropSubtitle')}
          </p>
        </div>

        {fotos.length === 0 ? (
          <p className="text-sm text-gray-500">{t('allianz.reportUnique.noPhotos')}</p>
        ) : (
