 ...
 */
export async function generarLiquidadorAllianzPdfBlob(liquidador) {
  const evalData = liquidador?.evaluacionSismicaNSR10 || {};
  const enc = liquidador?.encabezado || {};
  ...
  doc.setProperties({
    title: `Evaluación Sísmica NSR-10 Allianz - ${enc.siniestro || enc.asegurado || ''}`,
...
export async function descargarLiquidadorAllianzPdf(liquidador, totales) {
  const blob = await generarLiquidadorAllianzPdfBlob(liquidador, totales);
  const enc = liquidador?.encabezado || {};
