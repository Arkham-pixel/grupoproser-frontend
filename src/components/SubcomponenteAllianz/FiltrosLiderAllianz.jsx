...
import {
  OPCIONES_FECHA_FILTRO_ALLIANZ_CAT,
  OPCIONES_FECHA_FILTRO_ALLIANZ_LISTADO,
} from './allianzHelpers.js';

/**
 * Botón para que el líder elija el filtro (póliza o fechas) sin saturar la barra.
 */
export default function FiltrosLiderAllianz({
  variante = 'listado',
  filtroPoliza = '',
  onFiltroPoliza,
  filtroTipoPoliza = '',
  ...
  const fechas = variante === 'cat' ? OPCIONES_FECHA_FILTRO_ALLIANZ_CAT : OPCIONES_FECHA_FILTRO_ALLIANZ_LISTADO;
  ...
  const elegirPoliza = () => {
    ...
    setAbierto(false);
    window.setTimeout(() => document.getElementById('allianz-filtro-poliza')?.focus(), 0);
  };
  ...
  const elegirFecha = (value) => {
    onTipoFecha?.(value);
    setAbierto(false);
    const desde = document.getElementById('allianz-filtro-fecha-desde');
  ...
  return (
    <div className="space-y-4">
          ...
          {abierto && (
            ...
            >
              ...
              <p className="border-y border-gray-100 px-3 py-1.5 font-body text-[11px] font-semibold uppercase tracking-wide text-gray-400 dark:border-gray-800">
                {t('allianz.report.filterGroupDates')}
              </p>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {mostrarPoliza && (
          <Campo label={t('allianz.fields.numeroPoliza')}>
            <div className="flex gap-2">
              <InputFenix
                id="allianz-filtro-poliza"
                value={filtroPoliza}
                onChange={(e) => onFiltroPoliza?.(e.target.value)}
                placeholder={t('allianz.report.policyPlaceholder')}
              />
              <button
                type="button"
                ...
                title={t('allianz.report.removeFilter')}
              >
                <FaTimes />
              </button>
            </div>
        ...
        <Campo label={t('allianz.report.filterByDate')}>
