...
async function buildLogosHeader() {
  ...
  const right = new Paragraph({
    ...
    children: Allianz
      ...
      : [new TextRun({ text: 'Allianz', bold: true, font: 'Arial', size: 18 })],
  });
...
/**
 * Finiquito / Constancia de indemnización Allianz
 * (mismo esquema legal que Fundación de la Mujer / Equidad FDM),
 * con nombres de Allianz y logos Proser + Allianz.
 */
export async function descargarFiniquitoAllianzWord(liquidador = {}, totalesInput) {
  const enc = liquidador.encabezado || {};
  const totales = totalesInput || calcularLiquidacionAllianz(liquidador);
