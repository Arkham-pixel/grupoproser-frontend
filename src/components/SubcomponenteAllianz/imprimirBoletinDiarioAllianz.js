...
/**
 * @param {object} boletin - resultado de calcularBoletinDiarioAllianz
 * @param {object} labels - textos ya resueltos (i18n)
 */
export function construirHtmlBoletinDiarioAllianz(boletin, labels = {}) {
...
<head>
  <meta charset="utf-8" />
  <title>${esc(L.docTitle || 'Boletín diario Allianz')}</title>
  <style>${estilosImpresion()}</style>
</head>
<body>
  <p class="eyebrow">ALLIANZ</p>
 ...
 */
export function imprimirBoletinDiarioAllianz(boletin, labels = {}) {
  const html = construirHtmlBoletinDiarioAllianz(boletin, labels);
