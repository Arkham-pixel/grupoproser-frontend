...
async function construirBloqueDaniosUbicacionAllianz({ info = {}, caso = {} } = {}) {
  const bloques = [];
...
/**
 * Informe único Allianz — misma fórmula visual que Catastrófico/Puertos:
...
export async function descargarWordInformeAllianz({ caso = {}, informe = null, liquidador = null } = {}) {
  ...
  for (const archivo of fotosArchivos.slice(0, 12)) {
    const url = urlDescargaArchivoAllianz(archivo.ruta);
    if (!url) continue;
  ...
  const bloqueDaniosUbicacion = await construirBloqueDaniosUbicacionAllianz({ info, caso });
  ...
  const nombre = `Informe_Unico_ALLIANZ_${caso.siniestro || caso.consecutivo || 'caso'}.docx`.replace(
    /[^\w.\-áéíóúÁÉÍÓÚñÑ]+/gi,
    '_'
  );
  saveAs(blob, nombre);
