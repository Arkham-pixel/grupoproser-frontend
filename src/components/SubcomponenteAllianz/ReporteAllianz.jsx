...
const buildExportRow = (caso) => ({
  ...
  'FECHA PRIMER CONTACTO': formatDate(caso.fechaPrimerContacto),
  'FECHA INSPECCIÓN COORDINADA': formatDate(caso.fechaCoordinandoInspeccion),
  'FECHA INSPECCIÓN REALIZADA': formatDate(caso.fechaInspeccionRealizada),
  'FECHA ANÁLISIS DE CASO': formatDate(caso.fechaAnalisisCaso),
  ...
  'FECHA PENDIENTE APROBACIÓN ANALISTA': formatDate(caso.fechaAutorizacionAnalista),
  ...
  'FECHA DESISTIDO': formatDate(caso.fechaDesistido),
  'FECHA ANULADO/CANCELADO': formatDate(caso.fechaAnulado),
...
export default function ReporteAllianz() {
  ...
  const recargar = useCallback(async ({ silencioso = false } = {}) => {
    ...
    try {
      const data = await fetchAllCasosAllianz(2000, { soloChecklistLleno });
  ...
  const exportarExcel = () => {
    ...
    try {
      ...
      XLSX.writeFile(wb, `allianz-cat-${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (err) {
      setAviso({
        tipo: 'error',
        titulo: t('allianz.report.exportError'),
        mensaje: err.message || t('allianz.report.exportErrorMessage'),
      });
    }
  };
  ...
  const solicitarEliminar = (item) => {
      setAviso({
        ...
        onConfirm: async () => {
          try {
            await deleteCasoAllianz(item._id);
            setAviso({
              tipo: 'success',
      ...
      <div className={wrap}>
        <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-3">
            <span className={expressBadge}>Allianz · CAT</span>
            <div>
              <h1 className={expressPageTitle}>{t('allianz.report.title')}</h1>
              <p className={expressPageSubtitle}>
                {soloChecklistLleno
                  ? t('allianz.report.subtitleChecklist')
                  : t('allianz.report.subtitle')}
              </p>
            </div>
            <nav className="flex flex-wrap gap-2">
              <Link
                to="/allianz/dashboard"
              ...
              <tbody className="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-[#1A1A1A]">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={COLUMNAS.length + 1} className="px-4 py-8 text-center text-sm text-gray-500">
                      {soloChecklistLleno
                        ? t('allianz.report.noCasesChecklist')
