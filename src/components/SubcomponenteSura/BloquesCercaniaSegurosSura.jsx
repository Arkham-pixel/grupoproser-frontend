...
export default function BloquesCercaniaSegurosSura() {
  ...
  const cargar = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await getBloquesCercaniaSura({ radioKm: Number(radioKm) });
      setData(res);
      const open = {};
  ...
  const geocodeClienteYGuardar = async () => {
    ...
    if (!items.length) {
      return { ok: 0, failed: 0, message: t('segurosSura.bloques.nothingPending') };
    }
    await postUbicacionesPredioSura(items);
  ...
  const actualizarUbicaciones = async () => {
    ...
    try {
      let resumen;
      try {
        resumen = await postGeocodePendientesSura({ limit: 80, force: false });
  ...
  const resumenTxt = useMemo(() => {
    if (!data) return '';
    const base = t('segurosSura.bloques.summary', {
    ...
    if (data.omitidosInspeccionados > 0) {
      return `${base} · ${t('segurosSura.bloques.omittedInspected', {
      ...
      <div className={expressPageWrap}>
        <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h1 className={expressPageTitle}>{t('segurosSura.bloques.title')}</h1>
            <p className={expressPageSubtitle}>{t('segurosSura.bloques.subtitle')}</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {geocoding
                ? t('segurosSura.bloques.geocoding')
                : t('segurosSura.bloques.updateLocations')}
            </button>
          </div>
        </header>
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {bloques.length === 0 && sinUbicar.length === 0 && (
                  <p className="text-sm text-gray-500">{t('segurosSura.bloques.empty')}</p>
                )}
                      ...
                      >
                        <span className="flex items-center gap-2 font-heading font-semibold text-gray-900 dark:text-white">
                          ...
                          <span className="font-body text-sm font-normal text-gray-500">
                            ({bloque.cantidad}{' '}
                            {t('segurosSura.bloques.cases', { count: bloque.cantidad })})
                          </span>
                        </span>
                          ...
                          <table className="min-w-full text-left text-sm">
                            <thead>
                              <tr className="border-b border-gray-100 text-xs uppercase text-gray-500 dark:border-gray-800">
                                <th className="px-3 py-2">{t('segurosSura.bloques.colDistance')}</th>
                                <th className="px-3 py-2">{t('segurosSura.fields.siniestro')}</th>
                                <th className="px-3 py-2">{t('segurosSura.fields.asegurado')}</th>
                                <th className="px-3 py-2">{t('segurosSura.fields.direccionPredio')}</th>
                                <th className="px-3 py-2">{t('segurosSura.fields.ciudad')}</th>
                                <th className="px-3 py-2">{t('segurosSura.fields.estado')}</th>
                        ...
                        <table className="min-w-full text-left text-sm">
                          <thead>
                            <tr className="border-b border-amber-100 text-xs uppercase text-gray-500 dark:border-amber-900/40">
                              ...
                              <th className="px-3 py-2">{t('segurosSura.fields.direccionPredio')}</th>
                              <th className="px-3 py-2">{t('segurosSura.fields.ciudad')}</th>
                              <th className="px-3 py-2">{t('segurosSura.bloques.colStatus')}</th>
                              <th className="px-3 py-2" />
                            </tr>
                          </thead>
                          <tbody>
