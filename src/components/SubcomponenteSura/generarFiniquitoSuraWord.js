...
const ASEGURADORA = 'SEGUROS SURA S.A.';
...
async function buildLogosHeader() {
  ...
  const right = new Paragraph({
    ...
    children: sura
      ...
      : [new TextRun({ text: 'SEGUROS SURA', bold: true, font: 'Arial', size: 18 })],
  });
...
/**
 * Finiquito / Constancia de indemnización Seguros Sura
 * (mismo esquema legal que Fundación de la Mujer / Equidad FDM),
 * con nombres de Sura y logos Proser + Sura.
 */
export async function descargarFiniquitoSuraWord(liquidador = {}, totalesInput) {
  const enc = liquidador.encabezado || {};
  const totales = totalesInput || calcularLiquidacionSura(liquidador);
