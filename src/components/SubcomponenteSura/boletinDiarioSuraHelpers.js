/**
 * Helpers del boletín diario Seguros Sura (corte día a día, America/Bogota).
 * Independiente del boletín semanal.
 */

import { normalizarEstadoSura } from './segurosSuraHelpers.js';
import {
  isoDateBogota,
  parseFechaCaso,
  sumarDiasUtc,
...
const CORTES_KEY = 'segurosSura.boletinDiario.cortes';
...
export function casoIdSura(caso) {
...
export function clasificarCasoGestionTerremoto(caso = {}) {
  ...
  if (estado === 'ASIGNADO (PARA ASIGNAR INSPECTOR)') return 'enInspeccion';
  return 'verificacion';
}
...
export function clasificarGestionDiscriminada(caso = {}) {
  ...
  if (estado === 'ASIGNADO (PARA ASIGNAR INSPECTOR)') return 'solicitanInspeccion';
...
export function fechaIngresoCasoSura(caso = {}) {
  return (
...
export function casoExistiaAlCorte(caso, isoCorte) {
  const ingreso = fechaIngresoCasoSura(caso);
...
export function clasificarCasoAlCorte(caso = {}, isoCorte) {
  ...
  if (fechaIsoOnOrBefore(caso.fechaEnvioAseguradora, isoCorte) || fechaIsoOnOrBefore(caso.fechaLiquidado, isoCorte)) {
    const live = clasificarCasoGestionTerremoto(caso);
    if (live === 'liquidados' || live === 'pendientesPagoAlfa') return live;
    return 'pendientesPagoAlfa';
  }
  if (fechaIsoOnOrBefore(caso.fechaUltimoDocumento, isoCorte) || fechaIsoOnOrBefore(caso.fechaInspeccion, isoCorte)) {
    return 'enLiquidacion';
  }
...
export function calcularBoletinDiarioSura(casos = [], { fechaCorte = new Date(), persistirCorte = true } = {}) {
