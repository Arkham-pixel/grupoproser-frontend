...
/**
 * @param {object} boletin - resultado de calcularBoletinDiarioSura
 * @param {object} labels - textos ya resueltos (i18n)
 */
export function construirHtmlBoletinDiarioSura(boletin, labels = {}) {
...
<head>
  <meta charset="utf-8" />
  <title>${esc(L.docTitle || 'Boletín diario Seguros Sura')}</title>
  <style>${estilosImpresion()}</style>
</head>
<body>
  <p class="eyebrow">SEGUROS SURA</p>
 ...
 */
export function imprimirBoletinDiarioSura(boletin, labels = {}) {
  const html = construirHtmlBoletinDiarioSura(boletin, labels);
