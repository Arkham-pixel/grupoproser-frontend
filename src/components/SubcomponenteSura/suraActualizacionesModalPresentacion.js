/**
 * Presentación del modal de actualizaciones Sura (solo UI).
 */

import { formatCurrency, formatDate } from './segurosSuraHelpers.js';

export const SURA_FIELD_LABELS = Object.freeze({
  ...
  observacionReserva: 'Observación de reserva',
...
export function suraFieldLabel(field) {
  return SURA_FIELD_LABELS[field] || field;
}

export function isEmptySuraDisplayValue(value) {
  return value === null || value === undefined || value === '';
}

export function isPolicyPlaceholderDisplay(value) {
  if (isEmptySuraDisplayValue(value)) return false;
...
export function formatSuraPreviewValue(field, value) {
  if (isEmptySuraDisplayValue(value)) return 'Vacío';
...
export function formatSuraDateTime(value) {
  if (!value) return '—';
  try {
...
export function classifyChangeBadges(field, before, after) {
  const badges = [];
  if (field === 'siniestro' && isEmptySuraDisplayValue(before) && !isEmptySuraDisplayValue(after)) {
 ...
 */
export function normalizeSuraPreviewRows(payload) {
  ...
  return (rows || []).map((r) => {
    ...
    const changeEntries = Object.entries(changes).map(([field, ch]) => ({
      field,
      label: suraFieldLabel(field),
      before: ch?.before,
      after: ch?.after,
      beforeDisplay: formatSuraPreviewValue(field, ch?.before),
      afterDisplay: formatSuraPreviewValue(field, ch?.after),
...
export function buildSuraModalViewModel({ summary = {}, rows = [], source = {}, statusMeta = {} } = {}) {
  ...
  return {
    title: 'Seguros Sura — Actualizaciones disponibles',
    subtitle: 'Se detectaron cambios en el archivo de Control y Seguimiento.',
    fileName: source.fileName || '—',
    lastModifiedDisplay: formatSuraDateTime(source.lastModifiedDateTime),
    lastCheckedDisplay: formatSuraDateTime(
