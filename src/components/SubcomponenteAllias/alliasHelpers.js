...
export const ALLIAS_REPORTE_PAGE_SIZE = 25;

export const ESTADOS_ALLIAS = [
  'PENDIENTE',
  'EN INSPECCIÓN',
  'DOCUMENTACIÓN',
  'LIQUIDADO',
...
export function homologarEstadoAllias(valor) {
  const raw = String(valor || '').trim();
  if (!raw) return 'PENDIENTE';
  if (ESTADOS_ALLIAS.includes(raw)) return raw;
  const key = claveEstadoAllias(raw);
  const exacto = ESTADOS_ALLIAS.find((est) => claveEstadoAllias(est) === key);
  if (exacto) return exacto;
  return ESTADOS_ALLIAS_LEGACY[key] || raw;
}

/** Mismos tipos de documento que Complex. */
export const TIPOS_IDENTIFICACION_ALLIAS = [
  'CC',
  'CE',
  'NIT',
  'PASAPORTE',
...
export const TIPOS_POLIZA_ALLIAS = [
  'HOGAR',
  'INCENDIO',
  'TERREMOTO',
  'TODO RIESGO',
...
export const esTipoPolizaOtroAllias = (valor) =>
  /^OTROS?$/.test(String(valor || '').trim().toUpperCase());

export const etiquetaTipoPolizaAllias = (caso = {}) => {
  const tipo = String(caso.tipoPoliza || '').trim();
  const detalle = String(caso.tipoPolizaOtro || '').trim();
  if (esTipoPolizaOtroAllias(tipo) && detalle) return detalle;
  return tipo;
};

/** Tomadores base del consolidado Allias (columna TOMADOR). */
export const TOMADORES_ALLIAS_DEFAULT = [
  'BANCO AV VILLAS',
  'BANCO BOGOTA',
  'BANCO OCCIDENTE',
  'BANCO POPULAR',
...
export const ETIQUETAS_ARCHIVO_ALLIAS = [
  'GENERAL',
  'POLIZA',
  'INSPECCION',
  'LIQUIDACION',
...
/** Manual CAT Allias: severidad 1–6 (reporte de exposición). Textos exactos del Word. */
export const SEVERIDAD_CAT_ALLIAS = [
...
export const ACCESO_PREDIO_ALLIAS = ['SI', 'NO', 'PARCIAL'];
...
export const DISCLAIMER_CAT_ALLIAS =
  'Documento operativo para clasificar severidad, completar la base de exposición y registrar evidencia. No autoriza confirmar ni negar cobertura, prometer pagos ni actuar como vocero de Allias. La severidad es preliminar y no constituye liquidación del siniestro.';

export const labelSeveridadCat = (nivel) => {
  const n = Number(nivel);
  const found = SEVERIDAD_CAT_ALLIAS.find((s) => s.valor === n);
...
export const FORM_VACIO_ALLIAS = {
  siniestro: '',
  zc: '',
  identificacion: '',
  tipoIdentificacion: '',
...
export const TIPOS_NEGOCIO_HOMOLOGADO_ALLIAS = [
  'Lider Allias 100%',
  'coaseguro aceptado',
  'coaseguro cedido',
];
...
export const OPCIONES_SI_NO_ALLIAS = ['SI', 'NO'];

/** Grado de afectación (1–6), independiente de la severidad CAT del desprendible */
export const GRADOS_AFECTACION_ALLIAS = ['1', '2', '3', '4', '5', '6'];

export const normalizarSiNoAllias = (raw) => {
...
export const normalizarGradoAfectacionAllias = (raw) => {
...
export const CAMPOS_FECHA_Allias = [
  'fechaSiniestro',
  'fechaInicioPoliza',
  'fechaFinPoliza',
  'fechaInspeccion',
...
export const CAMPOS_NUMERICOS_ALLIAS = [
  'valorAseguradoInmueble',
  'valorAseguradoContenidos',
  'valorReservaPreventivaPromedio',
  'valorComercialInmueble',
...
export const CAMPOS_DECIMAL_ALLIAS = ['distanciaEpicentroKm'];
...
export const construirFormDesdecasoAllias = (caso = {}) => {
  const base = {
    ...FORM_VACIO_ALLIAS,
    ...Object.fromEntries(
      Object.keys(FORM_VACIO_ALLIAS)
        ...
        .map((clave) => {
          ...
          if (CAMPOS_FECHA_Allias.includes(clave)) return [clave, fechaParaInput(valor)];
          if (CAMPOS_NUMERICOS_ALLIAS.includes(clave)) return [clave, formatMiles(valor)];
          if (CAMPOS_DECIMAL_ALLIAS.includes(clave)) {
          ...
          if (clave === 'afectacion' || clave === 'lucroCesante') {
            return [clave, normalizarSiNoAllias(valor)];
          }
          if (clave === 'gradoAfectacion') {
            return [clave, normalizarGradoAfectacionAllias(valor)];
          }
  ...
  if (base.tipoPoliza && !TIPOS_POLIZA_ALLIAS.includes(base.tipoPoliza)) {
    if (!base.tipoPolizaOtro) base.tipoPolizaOtro = base.tipoPoliza;
    base.tipoPoliza = 'OTRO';
  }
  base.estado = homologarEstadoAllias(base.estado);
  return base;
};
