...
import { fetchAllCasosAllias, getAlertasAllias } from '../../services/alliasService.js';
import { formatCurrency } from './alliasHelpers.js';
import {
  calcularBoletinSemanalAllias,
  inicioSemanaBogota,
  shiftSemana,
  sumarDiasUtc,
} from './boletinSemanalAlliasHelpers.js';
import {
  expressBtnGhost,
  expressBtnPrimary,
  expressCard,
...
const NOTES_KEY = 'allias.boletinNotas';
...
export default function BoletinSemanalallias() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.error('Error cargando boletín Allias:', err);
        if (!cancelado) {
          setError(err.message || t('allias.boletin.loadError'));
          setCasos([]);
        }
      } finally {
  ...
  const boletin = useMemo(
    () => calcularBoletinSemanalAllias(casos, alertasPayload, { desde, hasta }),
      ...
      <div className={expressPageWrap}>
        <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between print:block">
          <div>
            ...
            <h1 className={expressPageTitle}>{t('allias.boletin.title')}</h1>
            <p className={expressPageSubtitle}>
              {t('allias.boletin.subtitle', { rango: boletin.rango.etiqueta })}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2 print:hidden">
            <button
              ...
              aria-label={t('allias.boletin.prevWeek')}
            >
              <FaChevronLeft /> {t('allias.boletin.prevWeek')}
            </button>
            <button
              type="button"
              className={expressBtnGhost}
            ...
            <button
              ...
              aria-label={t('allias.boletin.nextWeek')}
            >
              {t('allias.boletin.nextWeek')} <FaChevronRight />
            </button>
            <button type="button" className={expressBtnPrimary} onClick={() => window.print()}>
              <FaPrint /> {t('allias.boletin.print')}
            </button>
            <Link to="/allias/dashboard" className={expressBtnGhost}>
              {t('nav.alliasDashboard')}
            </Link>
            <Link to="/allias/reporte" className={expressBtnGhost}>
              {t('nav.alliasReport')}
            </Link>
          </div>
        </header>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <ExpressMetricCard
            label={t('allias.boletin.kpis.reported')}
            value={String(boletin.kpis.casosReportados)}
            hint={t('allias.boletin.kpis.reportedHint')}
          />
          <ExpressMetricCard
            label={t('allias.boletin.kpis.inspected')}
            value={String(boletin.kpis.casosInspeccionados)}
            hint={t('allias.boletin.kpis.inspectedHint')}
          />
          <ExpressMetricCard
            label={t('allias.boletin.kpis.settled')}
            value={String(boletin.kpis.casosLiquidados)}
            hint={t('allias.boletin.kpis.settledHint')}
          />
          <ExpressMetricCard
            label={t('allias.boletin.kpis.active')}
            value={`${boletin.kpis.casosActivos} / ${boletin.kpis.totalCasos}`}
            hint={t('allias.boletin.kpis.activeHint')}
          />
        </section>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <ExpressMetricCard
            label={t('allias.boletin.kpis.reserve')}
            value={formatCurrency(boletin.kpis.reservaEstimada)}
            hint={t('allias.boletin.kpis.reserveHint')}
          />
          <ExpressMetricCard
            label={t('allias.boletin.kpis.claimedActive')}
            value={formatCurrency(boletin.kpis.valorReclamadoActivos)}
            hint={t('allias.boletin.kpis.claimedActiveHint')}
          />
          <ExpressMetricCard
            label={t('allias.boletin.kpis.claimedWeek')}
            value={formatCurrency(boletin.kpis.valorReclamadoSemana)}
            hint={t('allias.boletin.kpis.claimedWeekHint')}
          />
          <ExpressMetricCard
            label={t('allias.boletin.kpis.adjustedWeek')}
            value={formatCurrency(boletin.kpis.valorAjustadoSemana)}
            hint={t('allias.boletin.kpis.adjustedWeekHint')}
          />
        </section>
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          <div className={expressCard}>
            ...
            <div className={`${expressCardBody} grid gap-4 sm:grid-cols-2`}>
              ...
              <div className="rounded-xl border border-gray-100 p-4 dark:border-gray-800">
                <p className="text-sm font-medium text-gray-500 dark:text-gray-400">
                  {t('allias.boletin.ans.settlement')}
                </p>
                <p className="mt-2 font-accent text-2xl font-bold text-gray-900 dark:text-white">
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          <div className={expressCard}>
            <div className={expressCardHeader}>
              <h2 className="font-heading text-lg font-bold text-gray-900 dark:text-white">
                {t('allias.boletin.news.title')}
              </h2>
            </div>
            <div className={expressCardBody}>
              {boletin.novedades.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {t('allias.boletin.news.empty')}
                </p>
              ) : (
              ...
              <label className="mt-4 block print:hidden">
                ...
                <textarea
                  ...
                  placeholder={t('allias.boletin.news.extraPlaceholder')}
                />
              </label>
          ...
          <div className={expressCard}>
            <div className={expressCardHeader}>
              <h2 className="font-heading text-lg font-bold text-gray-900 dark:text-white">
                {t('allias.boletin.risks.title')}
              </h2>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                {t('allias.boletin.risks.summary', {
            ...
            <div className={expressCardBody}>
              ...
              <label className="mt-4 block print:hidden">
                <span className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-gray-300">
                  {t('allias.boletin.risks.support')}
                </span>
                <textarea
                  className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-gray-700 dark:bg-[#1A1A1A] dark:text-gray-200"
                  rows={3}
                  ...
                  placeholder={t('allias.boletin.risks.supportPlaceholder')}
                />
              </label>
              {notas.apoyo && (
                <p className="mt-3 whitespace-pre-wrap text-sm text-gray-700 dark:text-gray-300">
                  <strong>{t('allias.boletin.risks.support')}:</strong> {notas.apoyo}
                </p>
              )}
            </div>
          </div>
            ...
            <table className="min-w-full text-left text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-xs uppercase text-gray-500 dark:border-gray-800">
                  <th className="px-2 py-2">{t('allias.boletin.detail.type')}</th>
                  <th className="px-2 py-2">{t('allias.fields.siniestro')}</th>
                  <th className="px-2 py-2">{t('allias.fields.asegurado')}</th>
                  <th className="px-2 py-2">{t('allias.fields.estado')}</th>
                  <th className="px-2 py-2">{t('allias.fields.ciudad')}</th>
                </tr>
              </thead>
              <tbody>
