...
async function buildLogosHeader() {
  ...
  const right = new Paragraph({
    ...
    children: Allias
      ...
      : [new TextRun({ text: 'Allias', bold: true, font: 'Arial', size: 18 })],
  });
...
/**
 * Finiquito / Constancia de indemnización Allias
 * (mismo esquema legal que Fundación de la Mujer / Equidad FDM),
 * con nombres de Allias y logos Proser + Allias.
 */
export async function descargarFiniquitoAlliasWord(liquidador = {}, totalesInput) {
  const enc = liquidador.encabezado || {};
  const totales = totalesInput || calcularLiquidacionAllias(liquidador);
  ...
  const safe = String(asegurado || siniestro || 'caso')
    ...
    .slice(0, 50);
  const nombre = `Finiquito_Constancia_Allias_${safe}.docx`;
  saveAs(blob, nombre);
}
