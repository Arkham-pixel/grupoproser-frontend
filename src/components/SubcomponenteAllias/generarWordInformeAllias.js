...
import { urlDescargaArchivoAllias } from '../../services/alliasService.js';
 ...
 * Logo Proser | Título + subtítulo + código/versión/fecha | Logo Allias
 */
async function crearEncabezadoAllias({ caso = {}, informe = {} } = {}) {
  ...
  const Allias =
    ...
    (await loadLogoBytes(`${base}templates/logo-allias.png`)) ||
        ...
        rows: [
          new TableRow({
            children: [
              ...
              logoCell(Allias, 'Allias', AlignmentType.RIGHT),
            ],
          }),
        ],
      }),
...
async function construirBloqueDaniosUbicacionAllias({ info = {}, caso = {} } = {}) {
  const bloques = [];
...
/**
 * Zona de firmas Allias: solo ajustador, sin tabla/bordes, abajo a la izquierda.
 */
async function construirZonaFirmasAllias({ info = {} } = {}) {
...
/**
 * Informe único Allias — misma fórmula visual que Catastrófico/Puertos:
 * encabezado formal, título ÚNICO, cuadro ficha, secciones y cuadros sin relleno.
 */
export async function descargarWordInformeAllias({ caso = {}, informe = null, liquidador = null } = {}) {
  const info = informe || defaultInformeUnicoAllias(caso);
  const liq = liquidador || mapcasoAlliasALiquidador(caso);
  const totales = calcularLiquidacionAllias(liq);
  const enc = liq.encabezado || {};
  const items = itemsPlanosAllias(liq);
  ...
  for (const archivo of fotosArchivos.slice(0, 12)) {
    const url = urlDescargaArchivoAllias(archivo.ruta);
    if (!url) continue;
  ...
  const header = await crearEncabezadoAllias({ caso, informe: info });
  ...
  const firmasParrafos = await construirZonaFirmasAllias({ caso, enc, info });
  const bloqueDaniosUbicacion = await construirBloqueDaniosUbicacionAllias({ info, caso });
  ...
  const nombre = `Informe_Unico_ALLIAS_${caso.siniestro || caso.consecutivo || 'caso'}.docx`.replace(
    /[^\w.\-áéíóúÁÉÍÓÚñÑ]+/gi,
    '_'
  );
  saveAs(blob, nombre);
