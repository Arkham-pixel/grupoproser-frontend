...
import {
  homologarEstadoAllias,
  normalizarGradoAfectacionAllias,
  normalizarSiNoAllias,
...
/** Mapeo de encabezados hoja BD / PENDIENTES / CAT_ALLIAS → campos del caso. */
const HEADER_MAP = {
  SINIESTRO: 'siniestro',
...
const parseFechaCelda = (valor) => {
  ...
  if (mdy) {
    ...
    let month = Number(a);
    let day = Number(b);
    // Si el primero > 12, es dd/mm; si no, en consolidado Allias suele ser m/d/yy (Excel US)
    if (month > 12 && day <= 12) {
      month = Number(b);
      day = Number(a);
    }
...
const parsearHojaACasos = (sheet) => {
  ...
  for (let r = headerRowIdx + 1; r < matriz.length; r += 1) {
    ...
    caso.estado = homologarEstadoAllias(caso.estado);
...
const elegirHojasCandidatas = (workbook) => {
  ...
  const baseAllias = nombres.find((n) => /BASE ALLIAS/i.test(n));
  if (baseAllias) orden.push(baseAllias);
  if (bd) orden.push(bd);
 ...
 */
export const parsearCasosAlliasDesdeExcel = async (file) => {
  ...
  if (/BASE ALLIAS/i.test(hojaUsada)) {
    casos = casos.map((caso) => ({
      ...caso,
      causa: caso.causa || 'TERREMOTO',
      canalRadicacion: caso.canalRadicacion || 'ALLIAS',
      tomador: caso.tomador || 'ALLIAS SEGUROS',
    }));
  }
 ...
 */
export const parsearListadoClienteAlliasDesdeExcel = async (file) => {
  if (!file) throw new Error('No se seleccionó archivo');
  const buffer = await file.arrayBuffer();
  const workbook = XLSX.read(buffer, { type: 'array', cellDates: true });
  if (!workbook.SheetNames?.length) throw new Error('El archivo no contiene hojas válidas');
