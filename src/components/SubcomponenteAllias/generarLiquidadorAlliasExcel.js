...
import { prefillNsrDesdecasoAllias } from './liquidadorAlliasHelpers.js';
 ...
 * con los datos del liquidador Allias.
 */
...
/**
 * Excel liquidador Allias = plantilla oficial Evaluación Sísmica NSR-10 rellenada.
 */
export async function generarLiquidadorAlliasExcelBlob(liquidador) {
  ...
  return {
    ...
    filename: `Evaluacion_Sismica_NSR10_Allias_${safe}.xlsx`,
  };
}

export async function descargarLiquidadorAlliasExcel(liquidador, totales) {
  const { blob, filename } = await generarLiquidadorAlliasExcelBlob(liquidador, totales);
