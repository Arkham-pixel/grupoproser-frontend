...
import ArchiveroAllias from './ArchiveroAllias.jsx';
...
export default function ReporteAllias() {
  ...
  const recargar = useCallback(async ({ silencioso = false } = {}) => {
    ...
    try {
      const data = await fetchAllCasosAllias(2000, { soloChecklistLleno });
  ...
  const exportarExcel = () => {
    if (!filtrados.length) {
      setAviso({ tipo: 'info', titulo: t('allias.report.noData'), mensaje: t('allias.report.noDataExport') });
      return;
    }
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Allias');
      XLSX.writeFile(wb, `allias-cat-${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (err) {
      setAviso({
        tipo: 'error',
        titulo: t('allias.report.exportError'),
        mensaje: err.message || t('allias.report.exportErrorMessage'),
      });
    }
  };
  ...
  const solicitarEliminar = (item) => {
      setAviso({
        ...
        onConfirm: async () => {
          try {
            await deleteCasoAllias(item._id);
            setAviso({
              tipo: 'success',
      ...
      <div className={wrap}>
        <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-3">
            <span className={expressBadge}>Allias · CAT</span>
            <div>
              <h1 className={expressPageTitle}>{t('allias.report.title')}</h1>
              <p className={expressPageSubtitle}>
                {soloChecklistLleno
                  ? t('allias.report.subtitleChecklist')
                  : t('allias.report.subtitle')}
              </p>
            </div>
              ...
              <tbody className="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-[#1A1A1A]">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={COLUMNAS.length + 1} className="px-4 py-8 text-center text-sm text-gray-500">
                      {soloChecklistLleno
                        ? t('allias.report.noCasesChecklist')
                    ...
                    <tr key={item._id} className="transition hover:bg-gray-50/80 dark:hover:bg-gray-900/30">
                      <td className="sticky left-0 z-20 whitespace-nowrap bg-white px-4 py-3 dark:bg-[#1A1A1A]">
                        <AccionesAlliasMenu
                          ...
                          onLiquidador={() =>
                            navigate(`/allias/liquidador?casoId=${item._id}&tab=liquidador`, {
                              state: { casoAllias: item },
                            })
                          }
                          onInformeUnico={() =>
                            navigate(`/allias/informe-unico?casoId=${item._id}&tab=informe`, {
                              state: { casoAllias: item },
                            })
                          }
