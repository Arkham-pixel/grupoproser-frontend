...
import { importarCasosAllias } from '../../services/alliasService.js';
import { importarCasosAlliasListado } from '../../services/alliasListadoService.js';
import {
  parsearCasosAlliasDesdeExcel,
  parsearListadoClienteAlliasDesdeExcel,
} from './importarAlliasExcel.js';
import {
  expressBtnGhost,
  expressBtnPrimary,
} from '../SubcomponenteExpress/expressFenixUi.js';

export function esAdminOSoporteAllias() {
...
/**
 * Carga masiva Allias:
 * - BASE ALLIAS / consolidado CAT → expedientes de inspección (liquidador e informe).
 * - Listado cliente (ZC, STRO) → módulo listado.
 */
export default function ModalImportarExcelAllias({ open, onClose, onCompleted }) {
  ...
  const handleFile = async (selected) => {
    ...
    try {
      ...
      let cat = null;
      try {
        listado = await parsearListadoClienteAlliasDesdeExcel(selected);
      } catch {
        listado = null;
      }
      try {
        cat = await parsearCasosAlliasDesdeExcel(selected);
      } catch {
        cat = null;
      }
      const esBaseAllias = /BASE ALLIAS/i.test(cat?.hoja || '');
      const usarCat =
        Boolean(cat?.casos?.length) &&
        (esBaseAllias || !listado?.casos?.length || cat.casos.length >= listado.casos.length);
      const parsed = usarCat ? cat : listado;
      if (!parsed?.casos?.length) {
        throw new Error(
          'No se encontraron filas válidas. Use BASE ALLIAS (siniestro, ID, asegurado) o el listado con ZC/STRO.'
        );
      }
  ...
  const handleImport = async () => {
    ...
    try {
      const data =
        modo === 'cat'
          ? await importarCasosAllias(casos)
          : await importarCasosAlliasListado(casos);
      const totals = {
        ...
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <p className="font-heading text-lg font-bold text-gray-900 dark:text-white">
              Importar Excel Allias
            </p>
            <p className="mt-1 font-body text-sm text-gray-500">
              {modo === 'cat'
                ? 'Formato BASE ALLIAS / consolidado: se crean o actualizan expedientes (liquidador e informe) por siniestro o identificación.'
