...
import logoAllias from '../../assets/zurich-logo.png';
import {
  actualizarArchivoAllias,
  eliminarArchivoAllias,
  getCasoAlliasById,
  guardarCatEnCasoAllias,
...
import { descargarDesprendibleCatAllias } from './generarDesprendibleCatAllias.js';
import { AUTOSAVE_DEBOUNCE_MS } from '../../config/autoSaveConfig.js';

const SEVERIDAD_MANUAL_CAT = SEVERIDAD_CAT_ALLIAS.map((s) => ({
  nivel: s.valor,
  descripcion: s.descripcion,
}));

const OBJETIVO_MANUAL =
  'Objetivo: entregar al ajustador información clave, sencilla y resumida para clasificar la severidad, completar la base de Excel y registrar evidencia fotográfica/documental. Este documento no autoriza a confirmar cobertura, negar cobertura, prometer pagos o actuar como vocero de Allias.';
 ...
 */
export default function InspeccionCatAllias({ casoAllias = null, onCasoChange }) {
  ...
  const [archivos, setArchivos] = useState(() => casoAllias?.archivos || []);
  const [observacionesCat, setObservacionesCat] = useState(
    () => casoAllias?.observacionesCat || ''
  );

  const [severidadNiveles, setSeveridadNiveles] = useState(() =>
    normalizeSeveridadCatNiveles(casoAllias?.severidadCatNiveles, casoAllias?.severidadCat)
  );
  ...
  useEffect(() => {
    ...
    setArchivos(casoAllias?.archivos || []);
  ...
  const persistirOrdenFotos = async (fotosOrdenadas) => {
    ...
    if (!casoAllias?._id || !ids.length) return;
    try {
      await reordenarArchivosAllias(casoAllias._id, ids);
    } catch (err) {
  ...
  const actualizarDescripcionFoto = (fotoId, descripcion) => {
    ...
    descripcionTimeoutRef.current[clave] = setTimeout(async () => {
      if (!casoAllias?._id || !clave || clave.startsWith('tmp-')) return;
      try {
        await actualizarArchivoAllias(casoAllias._id, clave, { descripcion });
      } catch (err) {
  ...
  const subirFotos = async (fileList) => {
    ...
    if (!casoAllias?._id) {
      setError(t('allias.cat.needSavedCase'));
      return;
    }
    setError('');
    setMensaje('');
    ...
    try {
      ...
      for (const file of files) {
        const creado = await subirArchivoAllias(casoAllias._id, file, 'FOTOS', {
          descripcion: '',
        });
      ...
      try {
        ...
        console.warn('Refresh archivos Allias:', refreshErr);
      }
      setMensaje(
        files.length === 1
          ? t('allias.archive.uploadOk')
  ...
  const buildCatPayload = () => {
    ...
    const fechaVisita =
      formatDateIso(casoAllias?.fechaInspeccion) || formatDateIso(new Date());
    const cat = {
      severidadCat,
  ...
  const persistirCat = async ({ silencioso = false } = {}) => {
    if (!casoAllias?._id) {
      if (!silencioso) setError(t('allias.cat.needSavedCase'));
      return null;
    }
    if (!silencioso) {
      setGuardando(true);
    ...
    try {
      ...
      const actualizado = await guardarCatEnCasoAllias({
        casoId: casoAllias._id,
        cat,
        casoBase: casoAllias,
      });
      if (!silencioso) {
      ...
      onCasoChange?.(actualizado);
      if (!silencioso) setMensaje(t('allias.cat.savedOk'));
      return actualizado;
    } catch (err) {
      if (!silencioso) setError(err.message || t('allias.cat.savedError'));
      return null;
    } finally {
  ...
  useEffect(() => {
    if (!casoAllias?._id) return undefined;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [severidadNiveles, observacionesCat, evidenciaCat, casoAllias?._id]);
  ...
  const guardarYDesprendible = async () => {
    ...
    try {
      const { cat } = buildCatPayload();
      const guardado = await guardarCatEnCasoAllias({
        casoId: casoAllias._id,
        cat,
        casoBase: casoAllias,
      });
      ...
      const base = {
        ...(guardado || {}),
        ...cat,
        archivos: archivos?.length ? archivos : guardado?.archivos || casoAllias.archivos || [],
      };
      const nombre = await descargarDesprendibleCatAllias(base);
  ...
  const exportarDesprendibleSinGuardar = async () => {
    ...
    try {
      ...
      const base = {
        ...(casoAllias || {}),
        ...cat,
        archivos: archivos?.length ? archivos : casoAllias?.archivos || [],
      };
      const nombre = await descargarDesprendibleCatAllias(base);
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4 border-b border-gray-200 pb-4 dark:border-gray-700">
        <div className="space-y-1">
          <p className="font-heading text-sm font-bold tracking-widest text-[#002060]">ALLIAS</p>
      ...
      {!casoAllias?._id ? (
        <div className={expressAlertError}>{t('allias.cat.needSavedCase')}</div>
      ) : null}
      ...
      <section>
        ...
        <label className="mb-1 block font-body text-sm font-semibold text-gray-800 dark:text-gray-200">
          {t('allias.fields.observacionesCat')}
        </label>
        <textarea
        ...
        <div
          ...
          onClick={() => !subiendo && casoAllias?._id && inputRef.current?.click()}
          onDragEnter={(e) => {
            e.preventDefault();
            e.stopPropagation();
            if (casoAllias?._id && !subiendo) setDragOver(true);
          }}
          onDragOver={(e) => {
            e.preventDefault();
            e.stopPropagation();
          ...
          } ${!casoAllias?._id || subiendo ? 'pointer-events-none opacity-60' : ''}`}
        >
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          onClick={guardarCat}
          disabled={guardando || generandoDoc || !casoAllias?._id}
        >
          <FaSave className="mr-2 inline" />
          {guardando ? t('allias.actions.saving') : t('allias.cat.saveCat')}
        </button>
        <button
          type="button"
