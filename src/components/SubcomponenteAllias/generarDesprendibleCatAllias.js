/**
 * Desprendible Manual CAT Allias (inspección / exposición).
...
import logoAlliasUrl from '../../assets/zurich-logo.png';
...
async function construirParrafosFotos(caso = {}) {
  ...
  for (let i = 0; i < fotosOrdenadas.length; i += 1) {
    ...
    const url = urlDescargaArchivoAllias(a.ruta);
...
/**
 * @param {object} caso — caso Allias (con severidadCatNiveles / evidenciaCat)
 */
export async function generarDesprendibleCatAllias(caso = {}) {
  ...
  const logoAllias = await loadLogoBytes(logoAlliasUrl);
            ...
            children: [
              new Paragraph({
                ...
                children: logoAllias
                  ? [
                      new ImageRun({
                        data: logoAllias.bytes,
                        transformation: { width: 110, height: 44 },
                        type: logoAllias.type,
                      }),
                    ]
                  : [
                      new TextRun({
                ...
                children: [
                  new TableCell({
                    ...
                    children: [
                      ...
                      p(DISCLAIMER_CAT_ALLIAS, { size: 15, italics: true, before: 40, after: 60 }),
                    ],
                  }),
                ],
              }),
...
export async function descargarDesprendibleCatAllias(caso) {
  const doc = await generarDesprendibleCatAllias(caso);
  const blob = await Packer.toBlob(doc);
  const nombre = `Desprendible_CAT_ALLIAS_${caso.consecutivo || caso.identificacion || 'caso'}.docx`;
  saveAs(blob, nombre);
  return nombre;
}
