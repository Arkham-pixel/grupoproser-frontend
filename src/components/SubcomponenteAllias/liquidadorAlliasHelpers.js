...
import { formatMiles } from './alliasHelpers.js';
import {
...
/** Texto fijo editable: información general del evento (consolidado terremoto Allias). */
export const INFO_EVENTO_DEFAULT_ALLIAS = `El presente informe se elabora en el marco de la atención del evento sísmico / catastrófico reportado ante Allias Seguros, conforme a la visita de inspección realizada al predio asegurado y a la documentación aportada por el tomador/asegurado.
...
export function crearItemAllias(item = '', valor = '', id) {
  return {
...
export function migrarItemLegacy(it = {}) {
  if (it.item != null || (it.valor != null && it.concepto == null)) {
    return crearItemAllias(it.item || '', it.valor ?? '', it.id);
  }
  const valor = it.valorIndemnizable || it.valorReclamado || '';
  return crearItemAllias(it.concepto || '', valor, it.id);
}
...
export function liquidacionCatastroficoDefaultAllias(caso = {}) {
...
export function encabezadoDesdecasoAllias(caso = {}) {
  const c = caso && typeof caso === 'object' ? caso : {};
  return {
    tomador: c.tomador || '',
...
/** Prefill portada NSR desde caso Allias */
export function prefillNsrDesdecasoAllias(caso = {}, encabezado = {}) {
  return {
...
export const DEFAULT_LIQUIDADOR_Allias = {
  modelo: 'nsr10',
  encabezado: {
    tomador: '',
    asegurado: '',
  ...
  liquidacionCatastrofico: liquidacionCatastroficoDefaultAllias(),
  indemnizacionSugerida: '',
  observaciones: '',
};

export function esLiquidadorNsrAllias(liquidador = {}) {
...
/**
 * Totales Allias = presupuesto NSR-10 + contenidos + diagrama (suma + hospedaje).
 * Compat: expone totalIndemnizar / totalIndemnizable para finiquito e informe.
 */
export function calcularLiquidacionAllias(liquidador = {}) {
...
export function itemsPlanosAllias(liquidador = {}) {
...
export function mapcasoAlliasALiquidador(caso = {}) {
  const encabezado = encabezadoDesdecasoAllias(caso);
  const prefill = prefillNsrDesdecasoAllias(caso, encabezado);
  const evalInicial = fusionarEvaluacionSismicaNSR10Guardada({}, prefill);
  const base = {
    ...DEFAULT_LIQUIDADOR_Allias,
    encabezado,
    evaluacionSismicaNSR10: evalInicial,
    liquidacionCatastrofico: liquidacionCatastroficoDefaultAllias(caso),
    valorReclamadoCaso:
  ...
  if (!esLiquidadorNsrAllias(guardado)) {
    return {
      ...base,
...
export function formDataNsrDesdeLiquidadorAllias(liquidador = {}, caso = {}) {
  const enc = liquidador.encabezado || {};
  return {
    ...prefillNsrDesdecasoAllias(caso, enc),
...
export function defaultInformeUnicoAllias(caso = {}) {
  const guardado =
  ...
  const base = {
    ...
    infoEvento: INFO_EVENTO_DEFAULT_ALLIAS,
    descripcionDanios: '',
    coordenadasRiesgo: '',
    imagenMapa: '',
