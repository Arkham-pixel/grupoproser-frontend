...
import { fetchAllCasosAllias, getCasoAlliasById } from '../../services/alliasService.js';
...
export default function ArchiveroAlliasPage() {
  ...
  useEffect(() => {
    ...
    async function cargar() {
      ...
      setError('');
      try {
        const actual = await getCasoAlliasById(casoIdFromQuery);
  ...
  const subtitulo = useMemo(() => {
    ...
    return t('allias.archive.pageSubtitle');
  }, [caso, t]);
      ...
      <div className={expressPageWrap}>
        ...
        {!casoId && !cargandoCaso && (
          <AlliasCasoPicker
            ...
            onSelect={elegirCaso}
            hint={t('allias.archive.pickCase')}
          />
        )}
        ...
        <div className={expressCard}>
          <div className={expressCardBody}>
            {cargandoCaso ? (
              ...
              <ArchiveroAllias caso={caso} onChanged={setCaso} />
            ) : (
              <p className="text-sm text-gray-500">{t('allias.archive.pickCase')}</p>
            )}
          </div>
        </div>
      </div>
