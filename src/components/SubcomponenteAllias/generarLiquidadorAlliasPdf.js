 ...
 */
export async function generarLiquidadorAlliasPdfBlob(liquidador) {
  const evalData = liquidador?.evaluacionSismicaNSR10 || {};
  const enc = liquidador?.encabezado || {};
  ...
  doc.setProperties({
    title: `Evaluación Sísmica NSR-10 Allias - ${enc.siniestro || enc.asegurado || ''}`,
...
export async function descargarLiquidadorAlliasPdf(liquidador, totales) {
  const blob = await generarLiquidadorAlliasPdfBlob(liquidador, totales);
  const enc = liquidador?.encabezado || {};
  const safe = String(enc.siniestro || enc.consecutivo || 'NSR10')
    .replace(/[^\w.-]+/g, '_')
    .slice(0, 40);
  saveAs(blob, `Evaluacion_Sismica_NSR10_Allias_${safe}.pdf`);
}
