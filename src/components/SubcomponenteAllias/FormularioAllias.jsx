...
import {
  crearCasoAlliasListado,
  actualizarCasoAlliasListado,
...
import {
  CAMPOS_NUMERICOS_ALLIAS,
  CAMPOS_DECIMAL_ALLIAS,
  ESTADOS_ALLIAS,
  FORM_VACIO_ALLIAS,
  TIPOS_IDENTIFICACION_ALLIAS,
  TIPOS_POLIZA_ALLIAS,
  esTipoPolizaOtroAllias,
  GRADOS_AFECTACION_ALLIAS,
  OPCIONES_SI_NO_ALLIAS,
  TIPOS_NEGOCIO_HOMOLOGADO_ALLIAS,
  construirFormDesdecasoAllias,
  formatMilesInput,
  normalizeEvidenciaCat,
} from './alliasHelpers.js';
import CampoTomadorAllias from './CampoTomadorAllias.jsx';
...
const AlliasRoot = 'min-h-full w-full min-w-0 bg-fenix-fondo dark:bg-[#0F0F0F] p-4 sm:p-6';
...
const FormularioAllias = ({ initialData = null, embed = false, origen = 'cat', onClose, onSaved }) => {
  const { t } = useTranslation();
  const rolUsuario = obtenerRolAlmacenado();
  const ctxPermiso = useMemo(() => obtenerContextoPermisoCaso('allias'), []);
  ...
  const [form, setForm] = useState(() =>
    initialData ? construirFormDesdecasoAllias(initialData) : { ...FORM_VACIO_ALLIAS }
  );
  ...
  useEffect(() => {
    setForm(initialData ? construirFormDesdecasoAllias(initialData) : { ...FORM_VACIO_ALLIAS });
    setError(null);
    setExito(null);
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      ...
      try {
        ...
        if (!esEdicion) {
          const liderDefault = resolverLiderPorModulo(lideresOpts, 'allias');
          if (liderDefault) {
            setForm((prev) =>
        ...
        if (!cancelado) console.error('Error cargando catálogos Allias:', err);
      } finally {
  ...
  const lideresAllias = useMemo(
    () => filtrarLideresPorModulo(responsables, 'allias'),
    [responsables]
  );
  ...
  const setCampo = (clave) => (e) => {
    ...
    setForm((prev) => {
      ...
      if (clave === 'tipoPoliza' && !esTipoPolizaOtroAllias(valor)) {
  ...
  const camposNumericos = useMemo(() => CAMPOS_NUMERICOS_ALLIAS, []);
  ...
  const construirPayload = () => {
    if (esModuloListado) {
      const payload = {
        ...
        tipoPolizaOtro: esTipoPolizaOtroAllias(form.tipoPoliza) ? form.tipoPolizaOtro : '',
        causa: form.causa,
    ...
    const payload = { ...form };
    if (!esTipoPolizaOtroAllias(payload.tipoPoliza)) payload.tipoPolizaOtro = '';
    camposNumericos.forEach((clave) => {
      payload[clave] = aNumero(payload[clave]);
    });
    CAMPOS_DECIMAL_ALLIAS.forEach((clave) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (esModuloListado) {
      if (!String(form.zc || '').trim() && !String(form.siniestro || '').trim()) {
        setError(t('allias.validation.zcOrStroRequired'));
        return;
      }
    } else if (!form.identificacion.trim() && !String(form.riskId || '').trim() && !String(form.zc || '').trim()) {
      setError(t('allias.validation.identificacionOrRiskRequired', {
        defaultValue: 'Indique identificación o Risk ID',
      }));
      return;
    }
    if (!form.estado.trim()) {
      setError(t('allias.validation.statusRequired'));
      return;
    }
    ...
    try {
      ...
      if (esEdicion) {
        guardado = esModuloListado
          ? await actualizarCasoAlliasListado(initialData._id, payload)
          : await actualizarCasoAllias(initialData._id, payload);
      } else {
        if (soloInspector) {
          setError(
            t('allias.permissions.inspectorCannotCreate', {
        ...
        guardado = esModuloListado
          ? await crearCasoAlliasListado(payload)
          : await crearCasoAllias(payload);
      }
      setExito(
        esEdicion
          ? t('allias.messages.caseUpdated', { caseNumber: guardado.consecutivo || '' })
          : t('allias.messages.caseCreated', { caseNumber: guardado.consecutivo || '' })
      );
      if (!esEdicion) {
        setForm({ ...FORM_VACIO_ALLIAS });
      }
      if (onSaved) await onSaved(guardado);
      await discardDraft();
    } catch (err) {
      console.error('Error guardando caso Allias:', err);
      setError(err.message || t('allias.messages.saveError'));
    } finally {
      setGuardando(false);
    }
  };
  ...
  const contenidoFormulario = (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <section className={expressFormSection}>
        <h3 className={expressSectionTitle}>{t('allias.sections.listadoCliente')}</h3>
        <p className="mb-3 font-body text-sm text-gray-600 dark:text-gray-400">
          {t('allias.sections.listadoClienteHint')}
        </p>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          <Campo label={t('allias.fields.zc')} required={esAltaCliente}>
            <InputFenix
              value={form.zc}
              onChange={setCampo('zc')}
              placeholder={t('allias.placeholders.zc')}
            />
          </Campo>
          <Campo label={t('allias.fields.siniestro')} required={esAltaCliente}>
            <InputFenix
              value={form.siniestro}
              onChange={setCampo('siniestro')}
              placeholder={t('allias.placeholders.siniestro')}
            />
          </Campo>
          <Campo label={t('allias.fields.asegurado')}>
            <InputFenix value={form.asegurado} onChange={setCampo('asegurado')} />
          </Campo>
          {esModuloListado && (
            <>
          <Campo label={t('allias.fields.telefonoAsegurado')}>
            <InputFenix
              value={form.telefonoAsegurado}
              onChange={setCampo('telefonoAsegurado')}
              placeholder={t('allias.placeholders.telefonoAsegurado')}
            />
          </Campo>
          <Campo label={t('allias.fields.correoAsegurado')}>
            <InputFenix
              type="email"
              value={form.correoAsegurado}
              onChange={setCampo('correoAsegurado')}
              placeholder={t('allias.placeholders.correoAsegurado')}
            />
          </Campo>
          <Campo label={t('allias.fields.tipoIdentificacion')}>
            {selectSimple('tipoIdentificacion', TIPOS_IDENTIFICACION_ALLIAS)}
          </Campo>
          <Campo label={t('allias.fields.identificacion')}>
            <InputFenix
              value={form.identificacion}
              onChange={setCampo('identificacion')}
              placeholder={t('allias.placeholders.identificacion')}
            />
          </Campo>
          <Campo label={t('allias.fields.numeroPoliza')}>
            <InputFenix
              value={form.numeroPoliza}
              onChange={setCampo('numeroPoliza')}
              placeholder={t('allias.placeholders.poliza')}
            />
          </Campo>
          <Campo label={t('allias.fields.tipoPoliza')}>
            {selectSimple('tipoPoliza', TIPOS_POLIZA_ALLIAS)}
          </Campo>
          {esTipoPolizaOtroAllias(form.tipoPoliza) && (
            <Campo label={t('allias.fields.tipoPolizaOtro')}>
              <InputFenix
                value={form.tipoPolizaOtro}
                onChange={setCampo('tipoPolizaOtro')}
                placeholder={t('allias.placeholders.tipoPolizaOtro')}
              />
            </Campo>
          )}
          <Campo
            label={t('allias.fields.causa')}
            className={esTipoPolizaOtroAllias(form.tipoPoliza) ? '' : 'md:col-span-2 lg:col-span-3'}
          >
            <InputFenix
              value={form.causa}
              onChange={setCampo('causa')}
              placeholder={t('allias.placeholders.causa')}
            />
          </Campo>
            </>
          )}
          <Campo label={t('allias.fields.intermediario')}>
            <InputFenix
              value={form.intermediario}
              onChange={setCampo('intermediario')}
              placeholder={t('allias.placeholders.intermediario')}
            />
          </Campo>
          <Campo label={t('allias.fields.correoIntermediario')}>
            <InputFenix
              type="email"
              value={form.correoIntermediario}
              onChange={setCampo('correoIntermediario')}
              placeholder={t('allias.placeholders.correoIntermediario')}
            />
          </Campo>
          <Campo label={t('allias.fields.telefonoIntermediario')}>
            <InputFenix
              value={form.telefonoIntermediario}
              onChange={setCampo('telefonoIntermediario')}
              placeholder={t('allias.placeholders.telefonoIntermediario')}
            />
          </Campo>
          <Campo label={t('allias.fields.ciudad')}>
            <SelectBuscable
              ...
              placeholder={t('allias.placeholders.selectCity')}
          ...
          <Campo label={t('allias.fields.estado')} required>
            {selectSimple('estado', ESTADOS_ALLIAS)}
          </Campo>
          <CamposAsignacionCaso
            form={form}
            setCampo={setCampo}
            lideres={lideresAllias}
            ...
            filtrarPorCiudad
          />
          <Campo label={t('allias.fields.fechaAsignacion')}>
            <InputFenix
              type="date"
          ...
          <Campo label={t('allias.fields.fechaVisita')}>
            <InputFenix
              type="date"
          ...
          <Campo label={t('allias.fields.observaciones')} className="md:col-span-2 lg:col-span-3">
            <textarea
              ...
              placeholder={t('allias.placeholders.observaciones')}
            />
          </Campo>
        </div>
        {soloInspector ? (
          <p className="mt-3 text-xs text-amber-700 dark:text-amber-300">
            {t('allias.permissions.inspectorHint', {
              defaultValue:
      ...
      {!esModuloListado && (
      <>
      {/* CAT Allias — campos del Excel CAT_ALLIAS */}
      <section className={expressFormSection}>
        <h3 className={expressSectionTitle}>{t('allias.sections.catAllias')}</h3>
        <p className="mb-3 font-body text-sm text-gray-600 dark:text-gray-400">
          {t('allias.sections.catAlliasHint')}
        </p>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          <Campo label={t('allias.fields.riskId')}>
            <InputFenix value={form.riskId} onChange={setCampo('riskId')} placeholder="Ej: 3518" />
          </Campo>
          <Campo label={t('allias.fields.distanciaEpicentroKm')}>
            <InputFenix
              type="text"
              inputMode="decimal"
          ...
          <Campo label={t('allias.fields.tipoNegocioHomologado')}>
            {selectSimple('tipoNegocioHomologado', TIPOS_NEGOCIO_HOMOLOGADO_ALLIAS)}
          </Campo>
          <Campo label={t('allias.fields.catUbicacionReferencia')}>
            <InputFenix
          ...
          <Campo label={t('allias.fields.addressNumber')}>
            <InputFenix value={form.addressNumber} onChange={setCampo('addressNumber')} />
          </Campo>
          <Campo label={t('allias.fields.direccionInspeccionSugerida')} className="md:col-span-2 lg:col-span-3">
            <InputFenix
              value={form.direccionInspeccionSugerida}
              onChange={setCampo('direccionInspeccionSugerida')}
            />
          </Campo>
          <Campo label={t('allias.fields.linkGoogleMaps')} className="md:col-span-2 lg:col-span-3">
            <InputFenix
          ...
          <Campo label={t('allias.fields.grupoInspeccion')}>
            <InputFenix value={form.grupoInspeccion} onChange={setCampo('grupoInspeccion')} />
          </Campo>
          <Campo label={t('allias.fields.afectacion')}>
            {selectSimple('afectacion', OPCIONES_SI_NO_ALLIAS)}
          </Campo>
          <Campo label={t('allias.fields.gradoAfectacion')}>
            {selectSimple('gradoAfectacion', GRADOS_AFECTACION_ALLIAS)}
          </Campo>
          <Campo label={t('allias.fields.lucroCesante')}>
            {selectSimple('lucroCesante', OPCIONES_SI_NO_ALLIAS)}
          </Campo>
          <Campo label={t('allias.fields.fechaInspeccion')}>
            <InputFenix
              type="date"
          ...
          <Campo label={t('allias.fields.observacionesCat')} className="md:col-span-2 lg:col-span-3">
            <textarea
              className="min-h-[88px] w-full rounded-lg border border-gray-300 bg-white px-3 py-2 font-body text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100"
      ...
      <details className={`${expressFormSection} group`}>
        <summary className="cursor-pointer list-none font-heading text-base font-bold text-gray-900 marker:content-none dark:text-white">
          <span className="inline-flex items-center gap-2">
            ...
            {t('allias.sections.generalInfo')}
          </span>
          <p className="mt-1 font-body text-sm font-normal text-gray-500">
            {t('allias.sections.generalInfoHint')}
          </p>
        </summary>
        ...
        <div className="mt-4 space-y-5 border-t border-gray-200 pt-4 dark:border-gray-700">
          <section>
            <h4 className="mb-3 font-heading text-sm font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-300">
              {t('allias.sections.identification')}
            </h4>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              <Campo label={t('allias.fields.siniestro')}>
                <InputFenix
                  value={form.siniestro}
                  onChange={setCampo('siniestro')}
                  placeholder={t('allias.placeholders.siniestro')}
              ...
              <Campo label={t('allias.fields.causa')}>
                <InputFenix
                  value={form.causa}
                  onChange={setCampo('causa')}
                  placeholder={t('allias.placeholders.causa')}
              ...
              />
              <Campo label={t('allias.fields.direccionPredio')}>
                <InputFenix value={form.direccionPredio} onChange={setCampo('direccionPredio')} />
              </Campo>
              <Campo label={t('allias.fields.informacionContacto')}>
                <InputFenix
                  value={form.informacionContacto}
                  onChange={setCampo('informacionContacto')}
                  placeholder={t('allias.placeholders.contacto')}
                />
              </Campo>
              <Campo label={t('allias.fields.correo')}>
                <InputFenix
                  type="email"
                  value={form.correo}
                  onChange={setCampo('correo')}
                  placeholder={t('allias.placeholders.correo')}
                />
              </Campo>
              <Campo label={t('allias.fields.celular')}>
                <InputFenix
                  value={form.celular}
                  onChange={setCampo('celular')}
                  placeholder={t('allias.placeholders.celular')}
                />
              </Campo>
              <Campo label={t('allias.fields.canalRadicacion')}>
                <InputFenix
                  value={form.canalRadicacion}
                  onChange={setCampo('canalRadicacion')}
                  placeholder="Ej: Allias"
                />
              </Campo>
              <Campo label={t('allias.fields.departamento')}>
                <SelectFenix
              ...
              <Campo label={t('allias.fields.ciudad')}>
                ...
                >
                  <option value="">
                    {form.departamento
                      ? t('allias.placeholders.selectCity')
                      : t('allias.placeholders.selectDepartmentFirst')}
                  </option>
          ...
          <section>
            <h4 className="mb-3 font-heading text-sm font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-300">
              {t('allias.sections.policyCoverage')}
            </h4>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              <Campo label={t('allias.fields.numeroPoliza')}>
                <InputFenix
              ...
              <Campo label={t('allias.fields.fechaInicioPoliza')}>
                <InputFenix
                  type="date"
              ...
              <Campo label={t('allias.fields.fechaFinPoliza')}>
                <InputFenix
                  type="date"
              ...
              <Campo label={t('allias.fields.numeroCredito')}>
                <InputFenix value={form.numeroCredito} onChange={setCampo('numeroCredito')} />
              </Campo>
              <Campo label={t('allias.fields.cobertura')}>
                <InputFenix
                  value={form.cobertura}
                  onChange={setCampo('cobertura')}
                  placeholder={t('allias.placeholders.cobertura')}
                />
              </Campo>
              <Campo label={t('allias.fields.estadoPagoPrimas')}>
                <InputFenix
                  value={form.estadoPagoPrimas}
                  onChange={setCampo('estadoPagoPrimas')}
                  placeholder={t('allias.placeholders.estadoPagoPrimas')}
                />
              </Campo>
            </div>
          </section>
          ...
          <section>
            <h4 className="mb-3 font-heading text-sm font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-300">
              {t('allias.sections.values')}
            </h4>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              <Campo label={t('allias.fields.valorAseguradoInmueble')}>
                {inputMiles('valorAseguradoInmueble')}
              </Campo>
              <Campo label={t('allias.fields.valorAseguradoContenidos')}>
                {inputMiles('valorAseguradoContenidos')}
              </Campo>
              <Campo label={t('allias.fields.valorReservaPreventivaPromedio')}>
                {inputMiles('valorReservaPreventivaPromedio')}
              </Campo>
              <Campo label={t('allias.fields.valorComercialInmueble')}>
                {inputMiles('valorComercialInmueble')}
              </Campo>
              <Campo label={t('allias.fields.reserva')}>{inputMiles('reserva')}</Campo>
              <Campo label={t('allias.fields.valorReclamado')}>
                {inputMiles('valorReclamado')}
              </Campo>
              <Campo label={t('allias.fields.valorLiquidado')}>
          ...
          <section>
            <h4 className="mb-3 font-heading text-sm font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-300">
              {t('allias.sections.dates')}
            </h4>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              <Campo label={t('allias.fields.fechaSiniestro')}>
                <InputFenix
                  type="date"
              ...
              <Campo label={t('allias.fields.fechaUltimoDocumento')}>
                <InputFenix
                  type="date"
              ...
              <Campo label={t('allias.fields.fechaLiquidado')}>
                <InputFenix
                  type="date"
              ...
              <Campo label={t('allias.fields.fechaAceptacionLiquidacion')}>
                <InputFenix
                  type="date"
              ...
              <Campo label={t('allias.fields.fechaEnvioAseguradora')}>
                <InputFenix
                  type="date"
      ...
      <div className="flex flex-col justify-end gap-2 sm:flex-row">
        ...
        <button type="button" className={expressBtnGhost} onClick={limpiar} disabled={guardando}>
          <FaUndo />
          {esEdicion ? t('allias.actions.reset') : t('allias.actions.clear')}
        </button>
        <button type="submit" className={expressBtnPrimary} disabled={guardando}>
          <FaSave />
          {guardando
            ? t('allias.actions.saving')
            : esEdicion
              ? t('allias.actions.saveChanges')
              : t('allias.actions.saveCase')}
        </button>
      </div>
    </form>
  );
  ...
  return (
    <div className={`${AlliasRoot} ${expressScope}`}>
      <div className={expressPageWrap}>
        <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-3">
            <span className={expressBadge}>{esModuloListado ? 'Allias · Listado' : 'Allias · CAT'}</span>
            <div>
              <h1 className={expressPageTitle}>{t('allias.page.addTitle')}</h1>
              <p className={expressPageSubtitle}>{t('allias.page.addSubtitle')}</p>
            </div>
            <nav className="flex flex-wrap gap-2">
              ...
              <Link
                to={esModuloListado ? '/allias/listado/reporte' : '/allias/reporte'}
              ...
              >
                {esModuloListado ? t('nav.alliasListadoReport') : t('nav.alliasReport')}
              </Link>
            </nav>
          </div>
        </header>
              ...
              {resumenImport && (
                <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                  <div className="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900">
                    <p className="font-body text-xs text-gray-500">{t('allias.bulk.received')}</p>
                    <p className="font-heading text-xl font-bold">{resumenImport.rows ?? 0}</p>
                  </div>
                  <div className="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900">
                    <p className="font-body text-xs text-gray-500">{t('allias.bulk.created')}</p>
                    <p className="font-heading text-xl font-bold text-fenix-primario">
                      {resumenImport.created ?? 0}
                    </p>
                  </div>
                  <div className="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900">
                    <p className="font-body text-xs text-gray-500">{t('allias.bulk.updated')}</p>
        ...
        <ModalImportarExcelAllias
          ...
          onCompleted={async (data) => {
            ...
            setExito(t('allias.bulk.success', {
        ...
        <section className={expressCard}>
          <div className={expressCardHeader}>
            <h2 className="font-heading text-lg font-bold text-gray-900 dark:text-white">
              {esEdicion
                ? t('allias.page.editCase', { caseNumber: initialData?.consecutivo || '' })
                : t('allias.page.newCase')}
            </h2>
          </div>
...
export default FormularioAllias;
