...
const a = Object.keys(es).filter((k) => k.startsWith('informeMadre.'));
const b = Object.keys(en).filter((k) => k.startsWith('informeMadre.'));
console.log('informeMadre es/en', a.length, b.length);
...
const files = walk('src/components/InformeMadre');
const used = new Set();
...
console.log(
  missing.length === 0 && a.length === b.length && a.length > 0 ? 'RESULT: OK' : 'RESULT: FAIL'
);
